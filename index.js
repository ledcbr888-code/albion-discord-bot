...
async function preloadDailyIcons() {
    await Promise.all(Object.keys(DAILY_CATEGORY_ITEM_IDS).map(category => loadDailyCategoryIcon(category)));
...
async function generateDailyBonusCard(serverChoice) {
    // Daily Bonus is intentionally Asia (East) only.
    serverChoice = 'asia';
    ...
    // Premium blue/gold Albion-style dashboard background.
    const bg = ctx.createLinearGradient(0, 0, width, height);
    bg.addColorStop(0, '#06111f');
    bg.addColorStop(0.45, '#0b1c31');
    bg.addColorStop(1, '#050a12');
    ctx.fillStyle = bg;
    ctx.fillRect(0, 0, width, height);

    // Decorative glow.
    const glow = ctx.createRadialGradient(width / 2, 150, 20, width / 2, 150, 520);
    glow.addColorStop(0, 'rgba(61,143,255,0.22)');
    ...
    ctx.strokeStyle = 'rgba(83,151,224,0.45)';
    ctx.lineWidth = 1;
    ...
    const statusText = dailyData.stale ? 'STALE DATA' : dailyData.source === 'COMMUNITY' ? 'COMMUNITY CONFIRMED' : 'CONFIRMED TODAY';

    // Header.
    ctx.fillStyle = '#f3c45b';
    ctx.font = '900 16px Arial, sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText('ALBION ONLINE', 44, 54);

    ctx.fillStyle = '#f7f3ea';
    ctx.font = '900 42px Arial, sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('DAILY PRODUCTION BONUS', width / 2, 56);

    drawRoundRect(ctx, 44, 76, 205, 34, 17);
    ctx.fillStyle = '#102d4e'; ctx.fill();
    ctx.strokeStyle = '#4f9be8'; ctx.lineWidth = 1; ctx.stroke();
    ctx.fillStyle = '#dceeff'; ctx.font = '900 14px Arial, sans-serif';
    ctx.fillText('🌐  ASIA (EAST)', 146, 99);

    drawRoundRect(ctx, width - 274, 76, 230, 34, 17);
    ctx.fillStyle = '#2b1d0a'; ctx.fill();
    ctx.strokeStyle = '#d8a642'; ctx.stroke();
    ctx.fillStyle = '#ffd36a'; ctx.font = '900 14px Arial, sans-serif';
    ctx.fillText(`⏱ NEXT RESET  ${nextReset.countdown}`, width - 159, 99);

    ctx.fillStyle = '#9fb0c5';
    ctx.font = 'bold 13px Arial, sans-serif';
    ctx.fillText(`BONUS TODAY  •  ${dateText}  •  ${statusText}`, width / 2, 132);
    ctx.textAlign = 'left';

    // Two equal bonus cards — the key change for days with two bonuses.
    const cardY = 154;
    const gap = 22;
    const cardX = 36;
    ...
    const iconSets = await Promise.all(entries.map(entry => loadDailyCategoryIcon(entry.category)));

    entries.forEach((entry, index) => {
        const x = cardX + index * (cardW + gap);
        const y = cardY;
        const accent = index === 0 ? '#4ea7ff' : '#e3a33f';
        const accentSoft = index === 0 ? 'rgba(78,167,255,0.16)' : 'rgba(227,163,63,0.16)';
        const border = entry.dailyBonus >= 20 ? '#ef9c42' : accent;

        drawRoundRect(ctx, x, y, cardW, cardH, 18);
        ctx.fillStyle = '#0d1b2c'; ctx.fill();
        ctx.strokeStyle = border; ctx.lineWidth = 2; ctx.stroke();

        // Card number + title.
        drawRoundRect(ctx, x + 20, y + 18, 42, 32, 16);
        ctx.fillStyle = accentSoft; ctx.fill();
        ctx.strokeStyle = accent; ctx.lineWidth = 1; ctx.stroke();
        ctx.fillStyle = accent; ctx.font = '900 15px Arial, sans-serif';
        ctx.textAlign = 'center'; ctx.fillText(`#${index + 1}`, x + 41, y + 40);

        ctx.fillStyle = '#91a6bf';
        ctx.font = '900 13px Arial, sans-serif';
        ctx.fillText('TODAY\'S CRAFTING BONUS', x + 82, y + 39);

        ctx.textAlign = 'left';
        const title = String(entry.category).toUpperCase();
        ctx.fillStyle = '#ffffff';
        ctx.font = `900 ${title.length > 18 ? 29 : 35}px Arial, sans-serif`;
        ctx.fillText(title, x + 24, y + 96, cardW - 48);

        // Main bonus pill.
        drawRoundRect(ctx, x + 24, y + 116, cardW - 48, 82, 16);
        ctx.fillStyle = accentSoft; ctx.fill();
        ctx.strokeStyle = accent; ctx.lineWidth = 1.5; ctx.stroke();
        ctx.fillStyle = '#8ea1b8';
        ctx.font = 'bold 12px Arial, sans-serif';
        ctx.fillText('CRAFTING BONUS', x + 42, y + 141);
        ctx.fillStyle = entry.dailyBonus >= 20 ? '#ffd27a' : '#f8d46e';
        ctx.font = '900 43px Arial, sans-serif';
        ctx.fillText(`+${entry.dailyBonus}%`, x + 42, y + 183);
        ctx.fillStyle = '#d7e0ea';
        ctx.font = '900 20px Arial, sans-serif';
        ctx.fillText('↑', x + cardW - 70, y + 175);

        // Large representative item icon.
        const iconData = iconSets[index];
        const iconX = x + 24;
        const iconY = y + 218;
        const iconSize = 220;
        drawRoundRect(ctx, iconX, iconY, iconSize, iconSize, 20);
        ctx.fillStyle = '#071321'; ctx.fill();
        ctx.strokeStyle = accent; ctx.lineWidth = 2; ctx.stroke();
        const iconGlow = ctx.createRadialGradient(iconX + iconSize/2, iconY + iconSize/2, 15, iconX + iconSize/2, iconY + iconSize/2, 110);
        iconGlow.addColorStop(0, index === 0 ? 'rgba(78,167,255,0.25)' : 'rgba(227,163,63,0.24)');
        iconGlow.addColorStop(1, 'rgba(0,0,0,0)');
        ctx.fillStyle = iconGlow; ctx.fillRect(iconX, iconY, iconSize, iconSize);
        if (iconData) {
            try { ctx.drawImage(iconData, iconX + 22, iconY + 22, iconSize - 44, iconSize - 44); } catch (_) { drawCategoryPlaceholder(ctx, entry.category, iconX + 25, iconY + 25, iconSize - 50); }
        } else {
            drawCategoryPlaceholder(ctx, entry.category, iconX + 25, iconY + 25, iconSize - 50);
        }

        // City information block.
        const infoX = x + 270;
        const infoY = y + 218;
        drawRoundRect(ctx, infoX, infoY, cardW - 294, 220, 18);
        ctx.fillStyle = '#101f31'; ctx.fill();
        ctx.strokeStyle = '#29445f'; ctx.lineWidth = 1; ctx.stroke();

        ctx.fillStyle = '#8da1b8';
        ctx.font = '900 12px Arial, sans-serif';
        ctx.fillText('BEST PLACE TO CRAFT', infoX + 18, infoY + 28);
        ctx.fillStyle = '#f5f6f7';
        ctx.font = `900 ${entry.city.length > 13 ? 21 : 25}px Arial, sans-serif`;
        ctx.fillText(entry.city.toUpperCase(), infoX + 18, infoY + 63, cardW - 330);
        ctx.fillStyle = '#f0c86a';
        ctx.font = 'bold 13px Arial, sans-serif';
        ctx.fillText(`CITY BONUS  +${entry.baseBonus}%`, infoX + 18, infoY + 91);

        // Bottom-right city crest, as requested.
        drawCityBadge(ctx, entry.city, infoX + (cardW - 294) - 86, infoY + 116, 68);
        ctx.fillStyle = '#7890aa';
        ctx.font = 'bold 11px Arial, sans-serif';
        ctx.fillText('BONUS CITY', infoX + 18, infoY + 145);
        ctx.fillStyle = '#dce8f4';
        ctx.font = 'bold 13px Arial, sans-serif';
        ctx.fillText('Craft here for the best local bonus', infoX + 18, infoY + 169, cardW - 330);
        ctx.fillStyle = '#61768e';
        ctx.font = 'bold 10px Arial, sans-serif';
        ctx.fillText('Representative item icon • T6 example', infoX + 18, infoY + 194, cardW - 330);

        // Small footer inside each card.
        ctx.fillStyle = '#71879f';
        ctx.font = 'bold 11px Arial, sans-serif';
        ctx.fillText(`BONUS #${index + 1}  •  +${entry.dailyBonus}% PRODUCTION`, x + 24, y + 565);
    });

    // Bottom summary strip.
    drawRoundRect(ctx, 36, 762, width - 72, 84, 18);
    ctx.fillStyle = '#0a1625'; ctx.fill();
    ctx.strokeStyle = '#35516d'; ctx.lineWidth = 1; ctx.stroke();
    ctx.fillStyle = '#f2c65f';
    ctx.font = '900 14px Arial, sans-serif';
    ctx.fillText('BONUS TODAY', 60, 790);
    const summary = entries.map(e => `${e.category}  +${e.dailyBonus}%`).join('   •   ');
    ctx.fillStyle = '#eef5fb';
    ctx.font = '900 18px Arial, sans-serif';
    ctx.fillText(summary, 60, 818, width - 360);
    ctx.fillStyle = '#8ea2b8';
    ctx.font = 'bold 12px Arial, sans-serif';
    ctx.textAlign = 'right';
    ctx.fillText(`ASIA (EAST)  •  NEXT RESET ${nextReset.countdown}`, width - 58, 790);
    ctx.fillStyle = '#d9c7a1';
    ctx.font = '900 15px Arial, sans-serif';
    ctx.fillText('POWERED BY  Boss Bot', width - 58, 818);
    ctx.textAlign = 'left';
...
async function checkAndSendDailyAutoReports() {
    ...
    for (const config of dailyAutoConfigs) {
        ...
        if (config.serverChoice !== 'asia') { config.serverChoice = 'asia'; saveData(); }
...
const commands = [
    ...
    new SlashCommandBuilder().setName('daily').setDescription('ดึงข้อมูลหรือตั้งค่ารายงานโบนัสประจำวันอัตโนมัติ')
        ...
        .addSubcommand(s => s.setName('setup').setDescription('ตั้งค่าให้บอทส่งรายงานโบนัส Asia (East) อัตโนมัติทุกวัน')
