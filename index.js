...
async function generateDailyBonusCard(serverChoice) {
    ...
    const iconTasks = entries.map(entry => Promise.all(
        (DAILY_CATEGORY_ITEM_IDS[entry.category] || []).map(loadDailyItemIcon)
    ));
