...
async function generateGuildSummaryImage(guildsData) {
  ...
  bg.addColorStop(0.55, "#111720");
  bg.addColorStop(1, "#05070a");
  ctx.fillStyle = bg;
  ctx.fillRect(0, 0, W, H);

  // Subtle fantasy/grid texture
  ...
  accent.addColorStop(0, "#d8a84e");
  accent.addColorStop(0.5, "#f0c66d");
  accent.addColorStop(1, "#8f6b2e");
  ...
  ctx.fillText("GUILD TRACKER", 42, 48);

  ctx.font = "700 34px Arial";
  ctx.fillStyle = "#f5f7fa";
  ctx.fillText("AUTO-BATTLE MONITOR", 42, 84);

  // Status badge
  const badgeW = 154;
  const badgeX = W - badgeW - 42;
  ctx.fillStyle = "#122719";
  roundRect(ctx, badgeX, 38, badgeW, 38, 19);
  ctx.fill();
  ctx.strokeStyle = "#2e7d4f";
  ctx.lineWidth = 1;
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(badgeX + 23, 57, 6, 0, Math.PI * 2);
  ctx.fillStyle = "#4ade80";
  ctx.fill();
  ctx.font = "700 14px Arial";
  ctx.fillStyle = "#9df2b8";
  ctx.fillText("ACTIVE", badgeX + 38, 62);

  const list = Array.isArray(guildsData) ? guildsData : [];
  ...
  const cardY = 112;
  const cardW = W - 84;
  const cardH = rows.length ? 76 : 116;
  ...
  if (!rows.length) {
    ...
    ctx.fillText("NO GUILDS TRACKED", cardX + 28, cardY + 48);

    ctx.font = "15px Arial";
    ctx.fillStyle = "#8993a1";
    ctx.fillText("Use /add guild to start monitoring a guild.", cardX + 28, cardY + 76);
  } else {
    rows.forEach((g, i) => {
      const y = cardY + i * (cardH + 10);

      ctx.fillStyle = "#0e131a";
      roundRect(ctx, cardX, y, cardW, cardH, 16);
      ctx.fill();
      ctx.strokeStyle = "#29313d";
      ctx.lineWidth = 1;
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.moveTo(ex + 20, ey);
      ctx.lineTo(ex + 38, ey + 8);
      ctx.lineTo(ex + 35, ey + 30);
      ctx.quadraticCurveTo(ex + 20, ey + 48, ex + 5, ey + 30);
      ctx.lineTo(ex + 2, ey + 8);
      ctx.closePath();
      const shield = ctx.createLinearGradient(ex, ey, ex + 40, ey + 40);
      shield.addColorStop(0, "#d7ad58");
      shield.addColorStop(1, "#715222");
      ctx.fillStyle = shield;
      ctx.fill();
      ctx.strokeStyle = "#f2d58e";
      ctx.stroke();

      ctx.font = "700 18px Arial";
      ctx.fillStyle = "#0a0d11";
      ctx.textAlign = "center";
      ctx.fillText(String(i + 1), ex + 20, ey + 27);
      ctx.textAlign = "left";
      ctx.restore();
      ...
      ctx.fillText(String(g.name || "Unknown Guild"), tx, y + 32);

      ctx.font = "13px Arial";
      ctx.fillStyle = "#7f8a99";
      ctx.fillText("REGION", tx, y + 54);
      ctx.font = "700 13px Arial";
      ctx.fillStyle = "#cbd5e1";
      ctx.fillText("EAST", tx + 56, y + 54);

      const channel = g.channelId ? `#${String(g.channelId).slice(0, 18)}` : "Discord Channel";
      ctx.font = "13px Arial";
      ctx.fillStyle = "#7f8a99";
      ctx.fillText("REPORT", tx + 118, y + 54);
      ctx.font = "700 13px Arial";
      ctx.fillStyle = "#cbd5e1";
      ctx.fillText(channel, tx + 169, y + 54);

      // Enabled pill
      const pillX = W - 206;
      ctx.fillStyle = "#151e28";
      roundRect(ctx, pillX, y + 20, 126, 34, 17);
      ctx.fill();
      ctx.strokeStyle = "#344150";
      ctx.stroke();
      ctx.font = "700 12px Arial";
      ctx.fillStyle = "#aeb8c5";
      ctx.fillText("AUTO REPORT ON", pillX + 17, y + 42);
    });
  }

  // Stats footer
  const statsY = rows.length ? Math.min(cardY + rows.length * (cardH + 10) + 18, 426) : 246;
  const stats = [
  ...
  const gap = 10;
  const statsW = (cardW - gap * (stats.length - 1)) / stats.length;
  stats.forEach((s, i) => {
    const x = cardX + i * (statsW + gap);
    ctx.fillStyle = "#0c1117";
    roundRect(ctx, x, statsY, statsW, 76, 14);
    ctx.fill();
    ...
    ctx.fillText(s[0], x + 15, statsY + 23);

    ctx.font = "700 25px Arial";
    ctx.fillStyle = i === 0 ? "#e6bf6a" : "#e8edf3";
    ctx.fillText(s[1], x + 15, statsY + 53);
  });

  // Footer
  ctx.font = "700 13px Arial";
  ctx.fillStyle = "#67717f";
  ctx.fillText("MONITORING ENABLED", 42, H - 30);

  ctx.textAlign = "right";
  ctx.font = "700 15px Arial";
  ctx.fillStyle = "#d6dbe2";
  ctx.fillText("POWERED BY", W - 166, H - 38);
  ctx.font = "700 18px Arial";
  ctx.fillStyle = "#f0c66d";
  ctx.fillText("Boss Bot", W - 42, H - 38);
  ctx.textAlign = "left";
...
async function generatePlayerWeaponReportImage(players) {
    ...
    // For large battles, switch from a long vertical table to a compact
    // landscape layout: 10 players per row, continuing left-to-right.
    ...
    const height = padding + headerHeight + rows * cardHeight + Math.max(0, rows - 1) * gap + 18;
    ...
    ctx.fillText(`Battle Report • ${players.length} Players`, padding, 30);
    ...
    players.forEach((p, i) => {
        ...
        ctx.beginPath();
        ctx.roundRect(x, y, cardWidth, cardHeight, 9);
        ctx.fill();
        ...
        if (name.length > 17) name = `${name.slice(0, 14)}...`;

        ctx.fillStyle = '#25b9ff';
        ctx.font = 'bold 15px sans-serif';
        ctx.fillText(name, x + 57, y + 21);

        ctx.font = 'bold 14px monospace';
        ctx.fillStyle = '#b58cff';
        ctx.fillText(`K ${p.kills || 0}`, x + 57, y + 42);

        ctx.fillStyle = p.deaths > 0 ? '#ff5b64' : '#68717d';
        ctx.fillText(`D ${p.deaths || 0}`, x + 115, y + 42);

        ctx.fillStyle = '#f5c84c';
        ctx.font = 'bold 13px monospace';
        ctx.fillText(formatFame(p.fame || 0), x + 57, y + 61);
    });
...
async function buildBattleReportPayload(matchId, customTargetGuilds = [], options = {}) {
    ...
    try {
        if (apiData.guilds) {
            // Count unique players who actually appear in this battle, grouped by Guild.
            // Do NOT use guild.memberCount because that can represent guild membership
            // rather than the number of participants in this specific battle.
        ...
        for (const config of autoBattleConfigs) {
            ...
            try {
                ...
                if (result.guildTotalFrames >= checkValue || result.totalFame >= checkValue) {
                    ...
                    if (channel) {
                        // 1. ส่งข้อความแจ้งเตือน (แก้ไขปัญหาแสดงลิงก์ไอคอนดิบๆ เรียบร้อยแล้ว)
                        await channel.send({
                            content: 
