...
async function generatePlayerWeaponReportImage(players, matchInfo = {}) {
    ...
    const width = Math.max(980, padding * 2 + contentWidth);
    ...
    ctx.fillText(`${players.length}`, playerCountBoxX + 65, 50);
    ctx.fillStyle = '#7a8b99';
    ctx.font = 'bold 10px sans-serif';
    ctx.fillText('PLAYERS', playerCountBoxX + 65, 68);

    // 3. Stat Bar 5 columns
    const statBarY = 100;
    const statBarWidth = width - (padding * 2);
    const statBarHeight = 62;
    ...
    const totalFame = players.reduce((acc, p) => acc + (p.fame || 0), 0);
    const topKiller = [...players].sort((a, b) => (b.kills || 0) - (a.kills || 0))[0] || { name: '-', kills: 0 };
    const topMvp = [...players].sort((a, b) => (b.fame || 0) - (a.fame || 0))[0] || { name: '-', fame: 0 };

    const colW = statBarWidth / 5;

    // Col 0: Total Kills
    ctx.textAlign = 'left';
    ctx.fillStyle = '#a855f7'; ctx.font = '20px sans-serif'; ctx.fillText('⚔️', padding + 15, statBarY + 38);
    ctx.fillStyle = '#7a8b99'; ctx.font = 'bold 10px sans-serif'; ctx.fillText('TOTAL KILLS', padding + 48, statBarY + 24);
    ctx.fillStyle = '#ffffff'; ctx.font = 'bold 20px sans-serif'; ctx.fillText(`${totalKills}`, padding + 48, statBarY + 48);

    // Col 1: Total Deaths
    const x1 = padding + colW;
    ctx.fillStyle = '#ff5555'; ctx.font = '20px sans-serif'; ctx.fillText('💀', x1 + 15, statBarY + 38);
    ctx.fillStyle = '#7a8b99'; ctx.font = 'bold 10px sans-serif'; ctx.fillText('TOTAL DEATHS', x1 + 48, statBarY + 24);
    ctx.fillStyle = '#ff5555'; ctx.font = 'bold 20px sans-serif'; ctx.fillText(`${totalDeaths}`, x1 + 48, statBarY + 48);

    // Col 2: Total Fame
    const x2 = padding + colW * 2;
    ctx.fillStyle = '#ffcc00'; ctx.font = '20px sans-serif'; ctx.fillText('👑', x2 + 15, statBarY + 38);
    ctx.fillStyle = '#7a8b99'; ctx.font = 'bold 10px sans-serif'; ctx.fillText('TOTAL FAME', x2 + 48, statBarY + 24);
    ctx.fillStyle = '#ffcc00'; ctx.font = 'bold 20px sans-serif'; ctx.fillText(`${formatFame(totalFame)}`, x2 + 48, statBarY + 48);

    // Col 3: Top Killer
    const x3 = padding + colW * 3;
    ctx.fillStyle = '#a855f7'; ctx.font = '20px sans-serif'; ctx.fillText('🎯', x3 + 12, statBarY + 38);
    ctx.fillStyle = '#7a8b99'; ctx.font = 'bold 10px sans-serif'; ctx.fillText('TOP KILLER', x3 + 42, statBarY + 20);
    ctx.fillStyle = '#ffffff'; ctx.font = 'bold 13px sans-serif';
    ctx.fillText(topKiller.name.length > 12 ? topKiller.name.slice(0, 10) + '..' : topKiller.name, x3 + 42, statBarY + 36);
    ctx.fillStyle = '#a855f7'; ctx.font = 'bold 10px sans-serif'; ctx.fillText(`${topKiller.kills} KILLS`, x3 + 42, statBarY + 50);

    // Col 4: MVP
    const x4 = padding + colW * 4;
    ctx.fillStyle = '#ffcc00'; ctx.font = '20px sans-serif'; ctx.fillText('🏆', x4 + 12, statBarY + 38);
    ctx.fillStyle = '#7a8b99'; ctx.font = 'bold 10px sans-serif'; ctx.fillText('MVP (TOP FAME)', x4 + 42, statBarY + 20);
    ctx.fillStyle = '#ffffff'; ctx.font = 'bold 13px sans-serif';
    ctx.fillText(topMvp.name.length > 12 ? topMvp.name.slice(0, 10) + '..' : topMvp.name, x4 + 42, statBarY + 36);
    ctx.fillStyle = '#ffcc00'; ctx.font = 'bold 10px sans-serif'; ctx.fillText(`${formatFame(topMvp.fame)} FAME`, x4 + 42, statBarY + 50);
    ...
    players.forEach((p, i) => {
        ...
        ctx.strokeStyle = i === 0 ? '#e6c662' : '#232b38';
        ctx.lineWidth = i === 0 ? 2 : 1;
        ctx.stroke();

        // Index number (Top Left)
        ctx.textAlign = 'left';
        ctx.fillStyle = i === 0 ? '#e6c662' : '#627385';
        ctx.font = 'bold 10px sans-serif';
        ctx.fillText(String(i + 1).padStart(2, '0'), x + 6, y + 14);

        // Crown icon for #1 (Top Right)
        if (i === 0) {
            ctx.fillText('👑', x + cardWidth - 18, y + 14);
        }
        ...
        // Player Name
        ctx.textAlign = 'center';
        ctx.fillStyle = i === 0 ? '#e6c662' : '#ffffff';
        ctx.font = 'bold 11px sans-serif';
        let displayName = p.name || '-';
        if (displayName.length > 10) displayName = displayName.slice(0, 8) + '..';
        ctx.fillText(displayName, x + cardWidth / 2, y + 72);
        ...
        ctx.fillText(killsText, x + cardWidth / 2 - 12, kdY);
        ctx.fillStyle = '#5a6b7c';
        ctx.fillText('/', x + cardWidth / 2, kdY);
        ctx.fillStyle = '#ff5555';
        ctx.fillText(deathsText, x + cardWidth / 2 + 12, kdY);

        // Fame Stats
        const fameY = y + 102;
        ctx.fillStyle = '#f5c84c';
        ctx.font = 'bold 9px monospace';
        ctx.fillText(`🛡️ ${formatFame(p.fame || 0)}`, x + cardWidth / 2, fameY);
    });
    ...
    ctx.fillText(matchInfo.targetGuildName ? `I ${matchInfo.targetGuildName} I` : 'I Flying Dutchman I', padding, footerY + 16);

    // Middle slogan
    ctx.textAlign = 'center';
    ctx.fillStyle = '#5a6b7c';
    ctx.font = 'bold 11px sans-serif';
    ctx.fillText('VICTORY BELONGS TO THOSE WHO FIGHT TOGETHER', width / 2, footerY + 8);

    // Right credit
    ctx.textAlign = 'right';
    ctx.fillStyle = '#7a8b99';
    ctx.font = 'bold 9px sans-serif';
    ctx.fillText('POWERED BY', width - padding, footerY);

    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 12px sans-serif';
    ctx.fillText('BossBot Bot', width - padding, footerY + 16);
...
async function buildBattleReportPayload(matchId, customTargetGuilds = [], options = {}) {
    ...
    try {
        const playerReport = await generatePlayerWeaponReportImage(displayRows, {
            matchId: matchId,
            time: formatUTCTime(battleTime),
            targetGuildName: guildNamesList[0] || ''
        });
