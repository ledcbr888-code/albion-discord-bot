...
const SERVER_ENDPOINTS = {
    west: 'https://game-info.albiononline.com/api/gameinfo/dailyactivity',
    asia: 'https://game-info-sgp.albiononline.com/api/gameinfo/dailyactivity',
    europe: 'https://game-info-ams.albiononline.com/api/gameinfo/dailyactivity'
};
...
async function fetchDailyBonus(serverKey) {
    const url = SERVER_ENDPOINTS[serverKey] || SERVER_ENDPOINTS.asia;
    try {
...
async function generateDailyBonusEmbed(serverChoice) {
    ...
    const descriptionText = apiData.description || apiData.details || 'ไม่มีรายละเอียดเพิ่มเติมจากเซิร์ฟเวอร์';
    const bonusType = apiData.productionBonus || apiData.bonusType || apiData.type || 'สุ่มโบนัสการผลิต/การเก็บเกี่ยวประจำวัน';
    let resetTimeFormatted = '10:00 UTC (17:00 น. ไทย)';
...
async function checkAndSendDailyAutoReports() {
    ...
    const now = new Date();
    // ดึงเวลาปัจจุบันในรูปแบบ UTC
    const utcHours = now.getUTCHours();
    const utcMinutes = now.getUTCMinutes();
    const todayStr = now.toISOString().split('T')[0];

    // รีเซ็ตโบนัสจะอยู่ที่ประมาณ 10:00 UTC (17:00 น. ไทย)
    // ให้บอทส่งรายงานอัตโนมัติช่วงเวลา 10:05 UTC (17:05 น. ไทย)
    if (utcHours === 10 && utcMinutes >= 5 && utcMinutes <= 10) {
        if (lastDailyReportDate === todayStr) return; // ส่งไปแล้วในวันนี้
...
const commands = [
    ...
    new SlashCommandBuilder().setName('daily').setDescription('ดึงข้อมูลหรือตั้งค่ารายงานโบนัสประจำวันอัตโนมัติ')
        .addSubcommand(s => s.setName('get').setDescription('ดึงข้อมูลโบนัสประจำวันทันที')
            ...
            .addChannelOption(o => o.setName('channel2').setDescription('เลือกห้องส่งเพิ่มเติมตัวที่ 2 (Optional)').addChannelTypes(ChannelType.GuildText)))
...
client.once('clientReady', async () => {
    ...
    setInterval(checkAndSendDailyAutoReports, 60 * 1000); // เช็กทุกๆ 1 นาที สำหรับ daily auto
});
...
client.on('interactionCreate', async interaction => {
    ...
    // ----------------------------------------
    // HANDLING /DAILY COMMAND (UPDATED)
    // ----------------------------------------
    if (commandName === 'daily') {
        ...
        if (sub === 'get') {
            ...
            // ดึงข้อมูล Daily Bonus จากทั้ง 3 เซิร์ฟเวอร์พร้อมกัน
            ...
            if (embeds.length === 0) {
                return interaction.editReply('❌ **เกิดข้อผิดพลาด:** ไม่สามารถเชื่อมต่อ API ดึงข้อมูลโบนัสรายวันของเซิร์ฟเวอร์ได้ในขณะนี้');
            }

            try {
                // ส่งไปยัง Channel หลัก
                ...
                // หากมีการเลือก Channel ที่ 2 ให้ส่งเพิ่มเติม
                if (targetChannel2) {
                    await targetChannel2.send({ embeds: embeds });
                    if (targetChannel1.id === interaction.channel.id) {
                        await interaction.followUp(`✅ ส่งรายงานโบนัสประจำวันไปยังห้องเพิ่มเติม <#${targetChannel2.id}> เรียบร้อยแล้ว!`);
                    }
                }
            } catch (err) {
