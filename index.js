...
async function generateDailyBonusCard(serverChoice) {
    ...
    entries.forEach((entry, index) => {
        ...
        ctx.fillText(title, x + 30, y + 172, cardW - 320);
        rounded(x + cardW - 168, y + 132, 126, 52, 26, '#4a2b0c', '#b87824');
        ctx.fillStyle = '#f0b34c';
        ctx.font = '900 27px Arial, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(`+${entry.dailyBonus}%`, x + cardW - 105, y + 168);
        ctx.textAlign = 'left';

        ctx.fillStyle = '#33251c';
        ctx.fillRect(x + 30, y + 195, cardW - 60, 1);
        ctx.fillStyle = '#b0a095';
        ctx.font = 'bold 14px Arial, sans-serif';
        ctx.fillText('BEST CRAFT CITY', x + 30, y + 224);
        ctx.fillStyle = '#eee7df';
        ctx.font = 'bold 23px Arial, sans-serif';
        ctx.fillText(`${entry.city}  +${entry.baseBonus}%`, x + 30, y + 258, cardW - 240);

        const iconData = iconSets[index] || { items: [], category: null };
        // Keep only one large, centered icon for the daily bonus category.
        if (iconData.category) {
            try { ctx.drawImage(iconData.category, x + cardW / 2 - 60, y + 42, 120, 120); } catch (_) {}
        }

        // Decorative crafting route panel uses a city badge at bottom-right.
        ctx.fillStyle = '#a99a8f';
        ctx.font = 'bold 14px Arial, sans-serif';
        ctx.fillText('CRAFTING ROUTE', x + 32, y + 310);
        ctx.fillStyle = '#c2a36a';
        ctx.font = 'bold 17px Arial, sans-serif';
        ctx.fillText('BONUS CITY VERIFIED', x + 32, y + 340);
        ctx.strokeStyle = '#46352a';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(x + 32, y + 358);
        ctx.lineTo(x + cardW - 150, y + 358);
        ctx.stroke();
