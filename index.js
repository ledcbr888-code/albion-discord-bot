...
async function fetchDailyBonus(serverKey) {
    ...
    if (aodpResult.status === 'fulfilled') {
        ...
        setDailySourceStatus(serverKey, { aodp: 'not_daily_or_waiting' });
...
async function generateBanditCard(event, minutesUntil) {
    ...
    if (banditIcon) {
        ...
        ctx.drawImage(banditIcon, 330, 49, 36, 36);
        ctx.restore();
    } else drawCaerleonCrest(ctx, 328, 49, 34);
    ...
    if (banditIcon) {
        ctx.save(); ctx.globalAlpha = 0.95; ctx.drawImage(banditIcon, rightX + 25, 710, 82, 82); ctx.restore();
    } else drawCaerleonCrest(ctx, rightX + 32, 716, 68);
    ...
    drawCaerleonCrest(ctx, 665, 970, 110);
    ...
    if (banditIcon) {
        ctx.save(); ctx.globalAlpha = 0.85; ctx.drawImage(banditIcon, 58, 1085, 34, 34); ctx.restore();
    } else drawCaerleonCrest(ctx, 58, 1085, 34);
