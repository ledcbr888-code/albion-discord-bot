...
async function handleImageTimer(message, imageUrl) {
    ...
    try {
        ...
        // 🖼️ นำข้อความที่สแกนได้มาสร้างรูปภาพจำลองพร้อมแปะป้ายเวลาให้ชัดเจน
        let imageAttachment = null;
        try {
            ...
            // วาดภาพต้นฉบับลงบน Canvas
            ctx.drawImage(baseImage, 0, 0);

            // วาดแถบพื้นหลังโปร่งแสงและกรอบสำหรับแสดงผลเวลาที่สแกนได้
            ...
            ctx.font = 'bold 22px sans-serif';
            ctx.fillText(`Unlocks in: ${h > 0 ? `${h}h ` : ''}${m}m ${s}s`, 20, baseImage.height - 18);
