...
function getWeaponRenderUrls(weapon, quality = 1) {
    ...
    const encoded = encodeURIComponent(id).replace(/%40/g, '@');
    const encodedBase = encodeURIComponent(baseId).replace(/%40/g, '@');
    return [
...
async function generatePlayerWeaponReportImage(players, battleInfo = {}) {
    ...
    const gridX = Math.floor((W - (cols * cardW + (cols - 1) * cardGapX)) / 2);
    const footerH = 56;
    ...
    // FIX: panelCut fills strictly only when 'fill' argument is provided
    ...
    sortedPlayers.forEach((p,i)=>{
        ...
        const x=gridX+col*(cardW+cardGapX), y=gridY+row*(cardH+cardGapY);
        ...
        if(isMVP){
            ctx.save();
            ctx.shadowColor='rgba(246,185,26,.45)';
            ctx.shadowBlur=22;
            panelCut(x+1,y+1,cardW-2,cardH-2,null,C.gold,9,1);
            ctx.restore();
        }
...
async function generateTopPerformanceImage(players) {
    ...
    const imagePromises = players.map(async (p) => {
        ...
        if (!weaponId || isBadEquipmentItem(weaponId)) return null;
        const urls = [
            `https://render.albiononline.com/v1/item/${encodeURIComponent(weaponId).replace(/%40/g, '@')}.png`,
            `https://render.albiononline.com/v1/item/${encodeURIComponent(weaponId.split('@')[0]).replace(/%40/g, '@')}.png`
        ];
        for (const url of urls) {
            try {
...
async function buildBattleReportPayload(matchId, customTargetGuilds = [], options = {}) {
    ...
    try {
        const [officialWeaponMap, albionBBWeaponMap] = await Promise.all([
            fetchOfficialBattleWeaponMap(matchId).catch(() => new Map()),
            fetchAlbionBBWeaponMap(matchId).catch(() => new Map())
        ]);
    ...
    let rows = allSortedRows.filter(p => {
    ...
    // FIX: Fallback to show all players if manually checking a fight outside tracked list
    if (rows.length === 0 && allSortedRows.length > 0) {
        rows = allSortedRows;
    }
...
async function fetchGuildRecentBattles(guildName) {
    try {
        ...
        const html = typeof searchRes === 'string' ? searchRes : (searchRes?.body || searchRes?.data || '');
        if (!html) return [];
...
async function checkAutoBattles() {
    ...
    try {
        for (const config of autoBattleConfigs) {
            ...
            // Check top 3 recent matches so no battles are missed
            const matchesToCheck = recentMatches.slice(0, 3);

            for (const matchId of matchesToCheck) {
                if (processedBattles.has(matchId)) continue;

                try {
                    const officialBattle = await fetchOfficialBattle(matchId);
                    if (!officialBattle) {
                        console.warn(`⚠️ Official Albion API ไม่พบ Match ID ${matchId}`);
                        continue;
                    }
                    ...
                    console.error(`❌ Auto-Battle Error on Match ID ${matchId} for guild ${config.targetGuild}:`, e.message);
                }
            }
