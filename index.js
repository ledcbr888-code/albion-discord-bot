...
// 🟢 ฟังก์ชันคำนวณตำแหน่ง Role และคืนค่า Emoji หน้าชื่อ
function getPlayerRole(weapon, damage = 0, healing = 0) {
    const w = normalizeAlbionItemId(weapon).toUpperCase();
    
    // Healers: มีค่า Healing หรือถือไม้ Holy / Nature (อิโมจิไม้กายสิทธิ์/คทาสีเขียว 🪄)
    if (healing > damage || w.includes('HOLY') || w.includes('NATURE')) {
        return { role: 'Healer', emoji: '🪄' };
    }
    
    // Tanks: ถืออาวุธสาย Tank (อิโมจิโล่สีฟ้า 🛡️)
    const tankKeywords = ['MACE', 'HAMMER', 'QUARTERSTAFF', 'CUBE'];
    if (tankKeywords.some(k => w.includes(k))) {
        return { role: 'Tank', emoji: '🛡️' };
    }
    
    // DPS: ทำความเสียหายหลัก (อิโมจิดาบสีแดง ⚔️)
    return { role: 'DPS', emoji: '⚔️' };
}
...
async function generateTopPerformanceImage(players) {
    ...
    for (let i = 0; i < players.length; i++) {
        ...
        // 🟢 ดึงข้อมูล Role และ Emoji ประจำตำแหน่ง
        const roleInfo = getPlayerRole(p.weapon, p.type === 'damage' ? p.value : 0, p.type === 'heal' ? p.value : 0);

        // 🟢 เพิ่มอิโมจิหน้าชื่อผู้เล่น (โดยไม่ต้องนำชื่อ Guild มาต่อรวม)
        let nameWithEmoji = `${roleInfo.emoji} ${p.name}`;
        if (nameWithEmoji.length > 28) nameWithEmoji = `${nameWithEmoji.slice(0, 25)}...`;
        
        ctx.fillStyle = '#000000';
        ctx.font = 'bold 20px sans-serif';
        ctx.fillText(nameWithEmoji, cardX + 94, y + 35);

        // 🟢 แยกชื่อ Guild มาแสดงแยกต่างหากด้านล่าง
        const guildBadge = p.guild ? `[${p.guild}] ` : '';
        ctx.font = 'bold 15px sans-serif';
        ctx.fillStyle = '#222222';
        ctx.fillText(`${guildBadge}${p.type === 'heal' ? 'HEAL' : 'DMG'} ${Number(p.value || 0).toLocaleString()} (${percent}%)`, cardX + 94, y + 65);
    }
...
async function processBattleReport(input, targetContext, isMessage = false) {
    try {
        ...
        // กรองเฉพาะผู้เล่นที่อยู่ใน targetPlayers หรือสังกัด targetGuilds เท่านั้น
        const rows = allSortedRows.filter(p => {
            if (targetGuilds.length === 0 && targetPlayers.length === 0) return false;

            const isGuildMatch = p.guild && targetGuilds.some(g => g.trim().toLowerCase() === p.guild.trim().toLowerCase());
            const isPlayerMatch = targetPlayers.some(pl => pl.trim().toLowerCase() === p.displayName.trim().toLowerCase());
            const isGuildAsNameMatch = targetGuilds.some(g => g.trim().toLowerCase() === p.displayName.trim().toLowerCase());

            return isGuildMatch || isPlayerMatch || isGuildAsNameMatch;
        });
        ...
        // ส่วนหัวรายงาน
        let header = `\x1b[1;36m⚔️ ALBIONBB BATTLE REPORT\x1b[0m | \x1b[1;33mID:\x1b[0m ${matchId}\n`;
        header += `\x1b[1;33m🕒 Time:\x1b[0m \x1b[1;37m${formatUTCTime(battleTime)}\x1b[0m\n`;
        header += '\x1b[30m==================================================\x1b[0m\n';
        ...
        if (rows.length === 0) {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                // 🟢 ดึงข้อมูล Role และตั้งค่า Emoji หน้าชื่อผู้เล่น
                const roleInfo = getPlayerRole(p.weapon, p.damage, p.healing);
                const nameWithEmoji = `${roleInfo.emoji} ${p.displayName}`;
                const name = nameWithEmoji.slice(0, widths.name - 1).padEnd(widths.name);
                ...
                const line = `${name}${p.kills > 0 ? `\x1b[32m${kills}\x1b[0m` : `\x1b[30m${kills}\x1b[0m`}${p.deaths > 0 ? `\x1b[31m${deaths}\x1b[0m` : `\x1b[30m${deaths}\x1b[0m`}${p.fame > 0 ? `\x1b[33m${fame}\x1b[0m` : `\x1b[30m${fame}\x1b[0m`}\n`;
