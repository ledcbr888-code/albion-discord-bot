...
async function generatePlayerWeaponReportImage(players, battleInfo = {}) {
    ...
    const gapX = 14, gapY = 14, padding = 28;
    const headerHeight = 88, statsHeight = 88;
    const gridOffsetY = padding + headerHeight + statsHeight + 22;
    ...
    function drawCrosshair(cx, cy, r, color) {
        ctx.save();
        ctx.strokeStyle = color; ctx.lineWidth = 1; ctx.globalAlpha = 0.32;
    ...
    function drawSwordIcon(cx, cy, scale, color) {
        ctx.save(); ctx.strokeStyle = color; ctx.lineWidth = 2.5 * scale; ctx.lineCap = 'round';
        ctx.beginPath(); ctx.moveTo(cx - 9*scale, cy + 9*scale); ctx.lineTo(cx + 8*scale, cy - 8*scale); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(cx - 8*scale, cy + 1*scale); ctx.lineTo(cx + 1*scale, cy + 10*scale); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(cx - 11*scale, cy + 7*scale); ctx.lineTo(cx - 3*scale, cy + 15*scale); ctx.stroke();
        ctx.restore();
    }

    function drawSkullIcon(cx, cy, scale, color) {
        ctx.save(); ctx.strokeStyle = color; ctx.fillStyle = color; ctx.lineWidth = 1.8 * scale;
        ctx.beginPath(); ctx.arc(cx, cy - 2*scale, 8*scale, Math.PI, 0); ctx.lineTo(cx + 7*scale, cy + 8*scale); ctx.lineTo(cx - 7*scale, cy + 8*scale); ctx.closePath(); ctx.stroke();
        ctx.fillRect(cx - 4*scale, cy + 7*scale, 8*scale, 4*scale);
        ctx.fillStyle = '#0a111d'; ctx.beginPath(); ctx.arc(cx - 3*scale, cy - 2*scale, 1.7*scale, 0, Math.PI*2); ctx.fill();
        ctx.beginPath(); ctx.arc(cx + 3*scale, cy - 2*scale, 1.7*scale, 0, Math.PI*2); ctx.fill();
        ctx.restore();
    }

    function drawCrownIcon(cx, cy, scale, color) {
        ctx.save(); ctx.strokeStyle = color; ctx.fillStyle = color; ctx.lineWidth = 1.7 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 9*scale, cy + 7*scale); ctx.lineTo(cx - 11*scale, cy - 6*scale);
        ctx.lineTo(cx - 4*scale, cy); ctx.lineTo(cx, cy - 8*scale);
        ctx.lineTo(cx + 4*scale, cy); ctx.lineTo(cx + 11*scale, cy - 6*scale);
        ctx.lineTo(cx + 9*scale, cy + 7*scale); ctx.closePath(); ctx.stroke();
        ctx.fillRect(cx - 8*scale, cy + 7*scale, 16*scale, 3*scale);
        ctx.restore();
    }

    function drawDiamond(cx, cy, r, color, fill = false) {
        ctx.save(); ctx.strokeStyle = color; ctx.lineWidth = 1.6;
    ...
    const glow = ctx.createRadialGradient(width*0.52, 110, 20, width*0.52, height*0.48, width*0.78);
    glow.addColorStop(0, '#18223a'); glow.addColorStop(0.42, '#0a1220'); glow.addColorStop(1, '#02050a');
    ctx.fillStyle = glow; ctx.fillRect(0, 0, width, height);

    // Fine grid + diagonal tactical lines
    ctx.save(); ctx.strokeStyle = 'rgba(100,116,139,0.10)'; ctx.lineWidth = 1;
    ...
    for (let x = -height; x < width; x += 180) { ctx.beginPath(); ctx.moveTo(x,0); ctx.lineTo(x+height,height); ctx.stroke(); }
    ctx.restore();

    // Premium top edge
    const topGrad = ctx.createLinearGradient(0,0,width,0);
    topGrad.addColorStop(0,'#7c3aed'); topGrad.addColorStop(0.38,'#c084fc'); topGrad.addColorStop(0.68,'#38bdf8'); topGrad.addColorStop(1,'#7c3aed');
    ctx.fillStyle = topGrad; ctx.fillRect(0,0,width,4);

    // ===== HEADER =====
    drawCrosshair(width - 88, 55, 24, '#a855f7');
    ctx.shadowColor = 'rgba(168,85,247,0.35)'; ctx.shadowBlur = 20;
    ctx.fillStyle = '#f8fafc'; ctx.font = '900 34px sans-serif';
    ctx.fillText('BATTLE REPORT', padding, padding + 34); ctx.shadowBlur = 0;
    ctx.fillStyle = '#64748b'; ctx.font = 'bold 11px sans-serif';
    ctx.fillText(`EAST SERVER   /   ${formatUTCTime(battleInfo.battleTime || Date.now())}   /   MATCH ${battleInfo.matchId || 'N/A'}`, padding+1, padding+56);
    ctx.fillStyle = '#a855f7'; ctx.font = 'bold 10px sans-serif';
    ctx.fillText('COMBAT ANALYTICS', padding+1, padding+73);

    // Players counter — more premium than a plain badge
    const badgeW=170,badgeH=60,badgeX=width-padding-badgeW,badgeY=24;
    rounded(badgeX,badgeY,badgeW,badgeH,14,'#09111f','#273b5c',1);
    ctx.fillStyle='#111d31'; drawRoundRect(ctx,badgeX+5,badgeY+5,48,badgeH-10,10); ctx.fill();
    ctx.fillStyle='#60a5fa'; ctx.font='900 23px sans-serif'; ctx.fillText(String(sortedPlayers.length),badgeX+17,badgeY+30);
    ctx.fillStyle='#64748b'; ctx.font='bold 9px sans-serif'; ctx.fillText('PLAYERS',badgeX+62,badgeY+23);
    ctx.fillStyle='#cbd5e1'; ctx.font='bold 11px sans-serif'; ctx.fillText('IN BATTLE',badgeX+62,badgeY+40);

    // ===== STAT STRIP =====
    const statY=padding+headerHeight, statGap=12;
    const statW=(width-padding*2-statGap*4)/5, statH=76;
    const stats=[
        {label:'TOTAL KILLS',value:totalKills.toLocaleString(),sub:'ELIMINATIONS',color:'#c084fc',icon:'sword'},
        {label:'TOTAL DEATHS',value:totalDeaths.toLocaleString(),sub:'CASUALTIES',color:'#fb7185',icon:'skull'},
        {label:'TOTAL FAME',value:formatFame(totalFame),sub:'KILL FAME',color:'#fbbf24',icon:'diamond'},
        {label:'TOP KILLER',value:String(topKiller.name).slice(0,14),sub:`${topKiller.kills} KILLS`,color:'#e879f9',icon:'crown'},
        {label:'MVP • TOP FAME',value:String(mvp.name).slice(0,14),sub:`${formatFame(mvp.fame)} FAME`,color:'#facc15',icon:'crown'}
    ];
    stats.forEach((st,i)=>{
        const x=padding+i*(statW+statGap);
        rounded(x,statY,statW,statH,12,'#09111e','#1b2a40',1);
        ctx.fillStyle=st.color; drawRoundRect(ctx,x,statY,3,statH,3); ctx.fill();
        const ix=x+22, iy=statY+31;
        if(st.icon==='sword') drawSwordIcon(ix,iy,0.75,st.color);
        if(st.icon==='skull') drawSkullIcon(ix,iy,0.72,st.color);
        if(st.icon==='diamond') drawDiamond(ix,iy,8,st.color,true);
        if(st.icon==='crown') drawCrownIcon(ix,iy,0.72,st.color);
        ctx.fillStyle='#64748b'; ctx.font='bold 9px sans-serif'; ctx.fillText(st.label,x+42,statY+22);
        const fs=textFit(st.value,statW-55,18,'900'); ctx.font=`900 ${fs}px sans-serif`; ctx.fillStyle='#f8fafc'; ctx.fillText(st.value,x+42,statY+44);
        ctx.fillStyle=st.color; ctx.font='bold 8px sans-serif'; ctx.fillText(st.sub,x+42,statY+60);
    });

    // Load real Albion weapon icons — keep the actual item art, upgrade the frame around it.
    ...
    sortedPlayers.forEach((p,i)=>{
        ...
        const x=padding+col*(cardWidth+gapX), y=gridOffsetY+row*(cardHeight+gapY);
        const isMVP=i===0, k=Number(p.kills)||0, d=Number(p.deaths)||0, q=Math.max(1,Math.min(5,Number(p.weaponQuality)||1));
        const qm=qualityMeta[q];
        const isFeeder=d>0&&k===0;

        // Card shell + subtle inner glow
        rounded(x,y,cardWidth,cardHeight,14,isMVP?'#111a2d':'#08101c',isMVP?'#f59e0b':(isFeeder?'#5b2330':'#1b2a40'),isMVP?1.8:1);
        if(isMVP){ ctx.save(); ctx.shadowColor='rgba(245,158,11,0.28)';ctx.shadowBlur=18;ctx.strokeStyle='#f59e0b';ctx.lineWidth=1.5;drawRoundRect(ctx,x,y,cardWidth,cardHeight,14);ctx.stroke();ctx.restore(); }

        // Rank marker
        rounded(x+9,y+9,32,20,6,isMVP?'#f59e0b':'#17243a');
        ctx.fillStyle=isMVP?'#111827':'#94a3b8';ctx.font='900 10px sans-serif';ctx.fillText(String(i+1).padStart(2,'0'),x+16,y+23);
        if(isMVP){
            ctx.fillStyle='#fbbf24';ctx.font='900 8px sans-serif';ctx.fillText('MVP',x+cardWidth-37,y+22);
        }

        // Weapon pedestal / hex-like layered frame
        const cx=x+cardWidth/2, cy=y+65, outer=42;
        ctx.save();
        ctx.shadowColor=qm.color;ctx.shadowBlur=isMVP?14:8;
        ctx.fillStyle='#040912';ctx.beginPath();ctx.arc(cx,cy,outer,0,Math.PI*2);ctx.fill();
        ctx.shadowBlur=0;
        ctx.strokeStyle=qm.color;ctx.lineWidth=2;ctx.beginPath();ctx.arc(cx,cy,outer,0,Math.PI*2);ctx.stroke();
        ctx.strokeStyle='rgba(148,163,184,0.22)';ctx.lineWidth=1;ctx.beginPath();ctx.arc(cx,cy,outer-7,0,Math.PI*2);ctx.stroke();
        ctx.restore();
        // Cardinal ticks make it look like a combat scanner
        ctx.save();ctx.strokeStyle=qm.color;ctx.lineWidth=2;ctx.globalAlpha=.65;
        [[0,-outer-2,0,-outer-8],[0,outer+2,0,outer+8],[-outer-2,0,-outer-8,0],[outer+2,0,outer+8,0]].forEach(a=>{ctx.beginPath();ctx.moveTo(cx+a[0],cy+a[1]);ctx.lineTo(cx+a[2],cy+a[3]);ctx.stroke();});ctx.restore();
        if(iconImages[i]){
            try{ctx.drawImage(iconImages[i],cx-34,cy-34,68,68);}catch(_){ }
        }else{
            drawSwordIcon(cx,cy,1.5,qm.color);
        }

        // Quality chip
        const qText=`Q${q}  ${qm.label}`;ctx.font='bold 7px sans-serif';const qw=ctx.measureText(qText).width+14;
        rounded(cx-qw/2,y+108,qw,16,8,'#070d17',qm.color,1);
        ctx.fillStyle=qm.color;ctx.font='900 7px sans-serif';ctx.fillText(qText,cx-qw/2+7,y+119);

        // Name
        let name=String(p.displayName||p.name||'Unknown'); if(name.length>19) name=`${name.slice(0,17)}..`;
        const nfs=textFit(name,cardWidth-22,13,'900');ctx.font=`900 ${nfs}px sans-serif`;ctx.fillStyle=isMVP?'#facc15':'#e2e8f0';
        ctx.fillText(name,x+(cardWidth-ctx.measureText(name).width)/2,y+140);

        // Guild tag
        const guild=String(p.guild||'').trim();
        if(guild){let g=guild.length>18?`${guild.slice(0,16)}..`:guild;ctx.font='bold 8px sans-serif';ctx.fillStyle='#64748b';ctx.fillText(g,x+(cardWidth-ctx.measureText(g).width)/2,y+152);}

        // K/D mini metrics
        const metricY=y+170;
        ctx.font='900 11px monospace';
        const kdText=`${k} / ${d}`, kdW=ctx.measureText(kdText).width;
        let mx=x+(cardWidth-kdW)/2;
        ctx.fillStyle=k>0?'#c084fc':'#64748b';ctx.fillText(String(k),mx,metricY);mx+=ctx.measureText(String(k)).width;
        ctx.fillStyle='#475569';ctx.fillText(' / ',mx,metricY);mx+=ctx.measureText(' / ').width;
        ctx.fillStyle=d>0?'#fb7185':'#64748b';ctx.fillText(String(d),mx,metricY);

        // Fame footer pill
        const fameText=formatFame(p.fame||0);ctx.font='bold 8px sans-serif';const fw=ctx.measureText(fameText).width+22;
        rounded(x+cardWidth-fw-9,y+cardHeight-20,fw,15,7,'#101827','#26364d',1);
        if(fameImg){try{ctx.drawImage(fameImg,x+cardWidth-fw-4,y+cardHeight-18,11,11);}catch(_){}}
        ctx.fillStyle='#fbbf24';ctx.font='900 8px sans-serif';ctx.fillText(fameText,x+cardWidth-fw+11,y+cardHeight-9);
    });

    // ===== FOOTER =====
    const footerY=height-padding-12;
    ctx.strokeStyle='#17263b';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(padding,footerY-23);ctx.lineTo(width-padding,footerY-23);ctx.stroke();
    ctx.fillStyle='#475569';ctx.font='900 9px sans-serif';ctx.fillText('VICTORY BELONGS TO THOSE WHO FIGHT TOGETHER',padding,footerY);
    ctx.fillStyle='#a855f7';ctx.font='900 9px sans-serif';const powered='POWERED BY  •  BOTBOSS';ctx.fillText(powered,width-padding-ctx.measureText(powered).width,footerY);
