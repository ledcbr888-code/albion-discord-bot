...
// Albion Database is a second, actively maintained community source for the
// daily-bonus category pool and crafting-location metadata. It does not expose
// a documented public endpoint for the live Asia draw, so it is used as a
// validation/source-health layer rather than inventing today's bonus.
const DAILY_BONUS_CATALOG_SOURCE = 'https://www.albiondatabase.com/events';
// Optional live JSON bridge. This lets the bot consume another trusted feed
// later without changing the bot code. Expected shape:
// { date:'YYYY-MM-DD', entries:[{category,city,baseBonus,dailyBonus}, ...] }
...
async function checkAlbionDatabaseCatalog(serverKey) {
    ...
    try {
        ...
        // This is only a diagnostic/catalog source. Cloudflare may return 403
        // to server-side clients; it must not be reported as a live draw error.
        if (status === 403) {
            setDailySourceStatus(serverKey, {
...
async function fetchDailyBonus(serverKey) {
    // Asia-only by design. Run the catalog check in parallel but never treat
    // its category list as today's live draw.
    ...
    const aodpPromise = aodpUrl ? axios.get(aodpUrl, {
        ...
        // AODP returns 404 when the server has no confirmed snapshot yet.
        // Treat that response as a normal source status, not a request error.
    ...
    // Keep source-health work non-blocking; it is only diagnostic metadata.
    ...
    // AODP's festivities endpoint is a separate activity schedule feed. It is
    // useful for diagnostics, but only accept it here when its payload actually
    // contains two recognized daily categories for today's date.
    ...
    // Same-day cache is safe to use as a temporary recovery only. Never roll
    // yesterday's Asia bonus forward into a new day.
    const cache = loadDailyCache();
...
async function generateDailyBonusCard(serverChoice) {
    ...
    // New premium fantasy-city background. The bundled image keeps the report
    // visually consistent on every hosting provider without external URLs.
    ...
    // Two equal bonus cards — one for each daily bonus.
    const gap = 18;
    const margin = 42;
    const cardY = 128;
    const cardH = 650;
    ...
    entries.forEach((entry, index) => {
        ...
        // Main content split: item image + city information.
        ...
        // Actual city badge at the bottom-right of each bonus card.
        ...
        // Explanation bar — deliberately short so nothing can overlap.
...
// Caerleon artwork is kept locally so Discord/CDN URL expiry cannot break cards.
...
function drawBanditIcon(ctx, image, x, y, size) {
    // The supplied Bandit artwork is a full card. Its top-left crest is used
    // as the compact icon while preserving the original artwork's gold rim.
    ctx.save();
    ctx.beginPath();
    ctx.arc(x + size / 2, y + size / 2, size / 2, 0, Math.PI * 2);
    ctx.clip();
...
async function generateBanditCard(event, minutesUntil) {
    // Portrait premium card based on the user's preferred reference:
    // hero artwork -> title -> status grid -> warning strip -> Caerleon footer.
    const width = 900;
    const height = 1200;
    const scale = 1.35;
    ...
    // Hero darkening + red cinematic wash
...
async function checkAndSendBanditAlerts() {
    ...
    // Alert at the 15-minute mark. Keep a small late-delivery grace window so
    // a scheduler tick delayed by a restart/network pause does not lose the
    // alert completely while the event is still about to start.
...
client.once('clientReady', async () => {
    ...
    // Run once immediately after Discord is ready, then poll every 30 seconds.
    // A one-minute interval can miss the 15-minute boundary during restarts or
    // event-loop pauses, especially when the process starts near that window.
