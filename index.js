...
async function generateBanditCard(event, minutesUntil) {
    ...
    ctx.fillStyle = 'rgba(7, 4, 10, 0.72)'; ctx.fillRect(0, 0, width, height);
    ...
    if (banditIcon) drawBanditIcon(ctx, banditIcon, 48, 30, 58);
    else drawCaerleonCrest(ctx, 52, 34, 50);
    if (caerleonCoin) {
        ctx.save();
        ctx.globalAlpha = 0.10;
        ctx.shadowColor = '#d6a34f';
        ctx.shadowBlur = 18;
        drawCoverImage(ctx, caerleonCoin, 760, 320, 130, 130);
        ctx.restore();
    }
