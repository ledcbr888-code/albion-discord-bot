...
// ฟังก์ชันสร้างรูปตารางสรุปกิลด์ (Guild Summary Canvas)
...
async function buildBattleReportPayload(matchId, customTargetGuilds = []) {
    ...
    try {
        if (apiData.guilds) {
            const guildsData = Object.values(apiData.guilds).map(g => ({
                name: g.name || 'Unknown',
    ...
    if (guildSummaryAttachment) {
        attachments.unshift(guildSummaryAttachment); // เอาภาพตารางกิลด์ไว้รูปแรก
    }
...
// ฟังก์ชันตรวจสอบ Auto-Battle (ดึงเฉพาะไฟต์ล่าสุดจริง ๆ และจำกัด Cache ป้องกันการรันซ้ำ)
async function checkAutoBattles() {
    ...
    try {
        for (const config of autoBattleConfigs) {
            ...
            // เอาเฉพาะ Match ID ล่าสุดตัวแรกสุดจากเว็บ
            const latestMatchId = recentMatches[0];

            // ถ้าเคยบันทึกไปแล้ว ข้ามไป
            ...
            try {
                ...
                if (result.guildTotalFrames >= checkValue || result.totalFame >= checkValue) {
                    ...
                    if (channel) {
                        await channel.send({
                            content: `🚨 **Auto-Battle Alert!** ตรวจพบไฟต์ใหม่ของกิลด์ **${config.targetGuild}**\n⚔️ รวม Kills + Deaths: \`${result.guildTotalFrames.toLocaleString()}\` | Total Fame: \`${formatFame(result.totalFame)}\``,
                            files: result.payload.files
                        });
                    }
                }
                
                // บันทึกว่า Match ID นี้ถูกตรวจและแจ้งเตือนไปแล้ว
            ...
            // จัดการขนาด Set ไม่ให้ใหญ่เกินไป (เก็บไว้ล่าสุด 300 รายการ)
...
client.once('clientReady', async () => {
    ...
    // ในตอนเริ่มต้น ให้ดึงรายการล่าสุดมาใส่ Cache ไว้ก่อน เพื่อไม่ให้บอทแจ้งเตือนไฟต์เก่าตกค้างตอนเพิ่งเปิดบอท
    setTimeout(async () => {
        for (const config of autoBattleConfigs) {
            const matches = await fetchGuildRecentBattles(config.targetGuild);
            if (matches.length > 0) {
                processedBattles.add(matches[0]); // จำเฉพาะอันล่าสุดสดๆ
            }
        }
