...
async function generateDailyBonusCard(serverChoice) {
    ...
    entries.forEach((entry, index) => {
        ...
        ctx.fillText(title, x + 148, y + 94, cardW - 320);
        ...
        ctx.fillText('BEST CRAFT CITY', x + 30, y + 148);
        ctx.fillStyle = '#eee7df';
        ctx.font = 'bold 17px Arial, sans-serif';
        ctx.fillText(`${entry.city}  +${entry.baseBonus}%`, x + 30, y + 174, cardW - 240);
        ...
        if (iconData.category) {
            try { ctx.drawImage(iconData.category, x + 34, y + 56, 92, 92); } catch (_) {}
        }
