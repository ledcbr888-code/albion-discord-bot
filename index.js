...
let autoBattleConfig = {
    channelId: null,
    minFame: 500000, // ค่าเริ่มต้น 500K Fame
    guilds: [],
    enabled: false
};
...
function loadData() {
    try {
        ...
        console.log(`📁 Tracking: ${targetGuilds.length} guilds, ${targetPlayers.length} players | AutoBattle: ${autoBattleConfig.enabled ? 'Enabled' : 'Disabled'}`);
    } catch (err) {
...
function saveData() {
    try {
        // จำกัดประวัติ Match ID ไว้ไม่เกิน 300 รายการล่าสุด ป้องกันไฟล์ใหญ่เกินไป
...
// 🟢 ปรับเปลี่ยนรองรับการส่งข้อความผ่าน Auto Notification
async function processBattleReport(input, targetContext, isMessage = false, isAuto = false) {
    try {
        ...
        const rows = allSortedRows.filter(p => {
            if (activeGuilds.length === 0 && targetPlayers.length === 0) return true; // หากไม่ได้ตั้งไว้ให้แสดงทั้งหมด
...
// 🟢 ระบบ Auto-Battle Polling (เช็กไฟต์ให้อัตโนมัติทุกๆ 5 นาที)
async function checkAutoBattles() {
    ...
    try {
        ...
        // ดึงรายการไฟต์ล่าสุดจาก Albion API (Asia Server)
        const urls = [
        ...
        for (const battle of battles) {
            ...
            if (processedBattles.includes(matchId)) continue; // ผ่านไฟต์ที่เคยส่งไปแล้ว

            const totalFame = Number(battle.totalFame || battle.killFame || 0);

            // เช็กชื่อกิลด์ที่เข้าร่วมไฟต์
...
const commands = [
    ...
    // 🟢 เพิ่มคำสั่ง /autobattle
    new SlashCommandBuilder().setName('autobattle').setDescription('ตั้งค่าระบบแจ้งเตือนไฟต์อัตโนมัติ (Auto Battle Notification)')
        .addSubcommand(s => s.setName('config').setDescription('ตั้งค่าห้องแจ้งเตือน, Fame ขั้นต่ำ และการเปิดใช้งาน')
            ...
            .addIntegerOption(o => o.setName('min_fame').setDescription('Fame ขั้นต่ำในการต่อสู้ (เช่น 500000 = 500K Fame)'))
            .addStringOption(o => o.setName('guild').setDescription('ชื่อกิลด์ที่ต้องการติดตาม (เว้นว่างไว้จะใช้กิลด์ทั้งหมดที่เคย /add guildไว้)'))
            .addBooleanOption(o => o.setName('enabled').setDescription('เปิด (True) / ปิด (False) ระบบแจ้งเตือน')))
        .addSubcommand(s => s.setName('status').setDescription('แสดงสถานะการตั้งค่าแจ้งเตือนปัจจุบัน'))
].map(c => c.toJSON());
...
client.once('ready', async () => {
    ...
    // 🟢 เริ่มทำงาน Loop เช็ก Auto Battle ทุกๆ 5 นาที (300,000 ms)
    setInterval(checkAutoBattles, 5 * 60 * 1000);
    setTimeout(checkAutoBattles, 10000); // ทำงานครั้งแรกหลังจากบอทเริ่มรัน 10 วินาที
});
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'autobattle') {
        ...
        if (sub === 'config') {
            ...
            const statusText = autoBattleConfig.enabled ? '🟢 เปิดใช้งาน (Enabled)' : '🔴 ปิดใช้งาน (Disabled)';
            const channelMention = autoBattleConfig.channelId ? `<#${autoBattleConfig.channelId}>` : 'ยังไม่ได้ตั้งค่า';
            const guildsText = autoBattleConfig.guilds.length > 0 ? autoBattleConfig.guilds.join(', ') : (targetGuilds.length > 0 ? `${targetGuilds.join(', ')} (จาก /add guild)` : 'ทุกกิลด์');

            const embed = new EmbedBuilder()
                .setColor(0x2ecc71)
                .setTitle('⚙️ อัปเดตการตั้งค่า Auto Battle Notification')
                .addFields(
                    ...
                    { name: '🛡️ กิลด์ที่ติดตาม', value: guildsText, inline: false }
                )
                .setTimestamp();
        ...
        if (sub === 'status') {
            ...
            const embed = new EmbedBuilder()
                .setColor(0x3498db)
                .setTitle('📊 สถานะระบบ Auto Battle Notification')
                .addFields(
                    { name: '📌 สถานะระบบ', value: statusText, inline: true },
                    { name: '📢 ห้องแจ้งเตือน', value: channelMention, inline: true },
                    { name: '🌟 Fame ขั้นต่ำ', value: `\`${formatFame(autoBattleConfig.minFame)}\``, inline: true },
