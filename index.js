...
async function generatePlayerWeaponReportImage(players, battleInfo = {}) {
    ...
    const headerHeight = 116, statsHeight = 92;
    ...
    // Focused guild / tracked guild
    const focusedGuild = String(battleInfo.guildName || '').trim();
    if (focusedGuild) {
        const guildLabel = focusedGuild.length > 38 ? `${focusedGuild.slice(0, 35)}...` : focusedGuild;
        ctx.fillStyle = '#fbbf24';
        ctx.font = '900 14px Arial, sans-serif';
        ctx.fillText(`⚔ ${guildLabel}`, padding + 1, padding + 58);
    }

    ctx.fillStyle = '#94a3b8'; ctx.font = 'bold 11px Arial, sans-serif';
    ctx.fillText(`EAST SERVER   |   ${formatUTCTime(battleInfo.battleTime || Date.now())}   |   MATCH ID: ${battleInfo.matchId || 'N/A'}`, padding + 1, padding + 77);
    ctx.fillStyle = '#ef4444'; ctx.font = '900 9px Arial, sans-serif';
    ctx.fillText('COMBAT ANALYTICS  /  LIVE BATTLE DATA', padding + 1, padding + 94);
    ...
    const stats = [
        ...
        { label: 'K / D RATIO', value: kdRatio, sub: 'COMBAT RATIO', color: '#60a5fa', icon: 'kills' },
    ...
    stats.forEach((st, i) => {
        ...
        const fs = textFit(val, statW - 78, (i === 2 ? 22 : (i >= 3 ? 18 : 22)), '900');
    ...
    sortedPlayers.forEach((p, i) => {
        ...
        // Player name — centered, high contrast and with subtle outline/shadow
        ...
        // Fame — FIXED: centered in the bottom row, larger and much easier to read
...
async function buildBattleReportPayload(matchId, customTargetGuilds = [], options = {}) {
    ...
    try {
        const playerReport = await generatePlayerWeaponReportImage(displayRows, {
            ...
            guildName: guildNamesList.length === 1 ? guildNamesList[0] : (guildNamesList.length > 1 ? guildNamesList.join(' / ') : '')
        });
