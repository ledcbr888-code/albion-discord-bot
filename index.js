...
// ฟังก์ชันดึง Event ล่าสุด (ปรับให้ดึงตรงจาก AlbionBB และมี Official API สำรอง)[cite: 8]
async function fetchRecentEvents() {
    ...
    // Fallback ใช้ Official API (SGP) ถ้า AlbionBB ขัดข้อง[cite: 8]
    try {
...
async function checkKillDeathTrackers() {
    if (!targetKillTrackers.length && !targetDeathTrackers.length) return;[cite: 8]
    ...
    for (const ev of events) {
        ...
        // 1. ตรวจสอบ Kill Trackers (ใช้ includes เพื่อให้ชื่อกิลด์ยืดหยุ่นขึ้น)[cite: 8]
        for (const tracker of targetKillTrackers) {
            ...
            if (tracker.type === 'player') {
                ...
                isMatch = killerGuild.toLowerCase().includes(tracker.name.toLowerCase()) || 
                          tracker.name.toLowerCase().includes(killerGuild.toLowerCase());
            }
            ...
            if (isMatch) {
                ...
                if (channel) {
                    const embed = new EmbedBuilder()
                        ...
                        .setTimestamp(new Date(ev.TimeStamp || ev.timeStamp || Date.now()));
                    await channel.send({ embeds: [embed] }).catch(() => {});
                }
            }
        }

        // 2. ตรวจสอบ Death Trackers[cite: 8]
        for (const tracker of targetDeathTrackers) {
            let isMatch = false;
            if (tracker.type === 'player') {
                isMatch = victimName.toLowerCase() === tracker.name.toLowerCase();
            } else if (tracker.type === 'guild') {
                isMatch = victimGuild.toLowerCase().includes(tracker.name.toLowerCase()) || 
                          tracker.name.toLowerCase().includes(victimGuild.toLowerCase());
            }
