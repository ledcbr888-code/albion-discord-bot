...
const DAILY_CATEGORY_ITEM_IDS = {
    // Real Albion item IDs used as representative icons for each bonus category.
    // The first ID is used for the large center icon; later IDs are fallbacks.
    'Cloth Robes': ['T6_CLOTH_ROBE', 'T5_CLOTH_ROBE', 'T4_CLOTH_ROBE'],
    'Leather Jackets': ['T6_LEATHER_JACKET', 'T5_LEATHER_JACKET', 'T4_LEATHER_JACKET'],
    'Cloth Cowl': ['T6_CLOTH_COWL', 'T5_CLOTH_COWL', 'T4_CLOTH_COWL'],
    'Cloth Sandals': ['T6_CLOTH_SHOES', 'T5_CLOTH_SHOES', 'T4_CLOTH_SHOES'],
    'Crossbow': ['T6_2H_CROSSBOW', 'T5_2H_CROSSBOW', 'T4_2H_CROSSBOW'],
    'Cursed Staff': ['T6_2H_CURSEDSTAFF', 'T5_2H_CURSEDSTAFF', 'T4_2H_CURSEDSTAFF'],
    'Dagger': ['T6_MAIN_DAGGER', 'T5_MAIN_DAGGER', 'T4_MAIN_DAGGER'],
    'Plate Armor': ['T6_PLATE_ARMOR', 'T5_PLATE_ARMOR', 'T4_PLATE_ARMOR'],
    'Stone': ['T6_STONEBLOCK', 'T5_STONEBLOCK', 'T4_STONEBLOCK'],
    'Hammer': ['T6_2H_HAMMER', 'T5_2H_HAMMER', 'T4_2H_HAMMER'],
    'Holy Staff': ['T6_MAIN_HOLYSTAFF', 'T5_MAIN_HOLYSTAFF', 'T4_MAIN_HOLYSTAFF'],
    'Plate Helmet': ['T6_PLATE_HEAD', 'T5_PLATE_HEAD', 'T4_PLATE_HEAD'],
    'Spear': ['T6_MAIN_SPEAR', 'T5_MAIN_SPEAR', 'T4_MAIN_SPEAR'],
    'Wood': ['T6_WOOD', 'T5_WOOD', 'T4_WOOD'],
    'Arcane Staff': ['T6_MAIN_ARCANESTAFF', 'T5_MAIN_ARCANESTAFF', 'T4_MAIN_ARCANESTAFF'],
    'Bow': ['T6_2H_BOW', 'T5_2H_BOW', 'T4_2H_BOW'],
    'Fiber': ['T6_FIBER', 'T5_FIBER', 'T4_FIBER'],
    'Leather Hood': ['T6_LEATHER_HEAD', 'T5_LEATHER_HEAD', 'T4_LEATHER_HEAD'],
    'Leather Shoes': ['T6_LEATHER_SHOES', 'T5_LEATHER_SHOES', 'T4_LEATHER_SHOES'],
    'Sword': ['T6_MAIN_SWORD', 'T5_MAIN_SWORD', 'T4_MAIN_SWORD'],
    'Axe': ['T6_MAIN_AXE', 'T5_MAIN_AXE', 'T4_MAIN_AXE'],
    'Frost Staff': ['T6_MAIN_FROSTSTAFF', 'T5_MAIN_FROSTSTAFF', 'T4_MAIN_FROSTSTAFF'],
    'Hide': ['T6_HIDE', 'T5_HIDE', 'T4_HIDE'],
    'Off-Hand': ['T6_OFFHAND', 'T5_OFFHAND', 'T4_OFFHAND'],
    'Plate Boots': ['T6_PLATE_BOOTS', 'T5_PLATE_BOOTS', 'T4_PLATE_BOOTS'],
    'Quarterstaff': ['T6_2H_QUARTERSTAFF', 'T5_2H_QUARTERSTAFF', 'T4_2H_QUARTERSTAFF'],
    'Fire Staff': ['T6_MAIN_FIRESTAFF', 'T5_MAIN_FIRESTAFF', 'T4_MAIN_FIRESTAFF'],
    'Leather Jackets': ['T6_LEATHER_JACKET', 'T5_LEATHER_JACKET', 'T4_LEATHER_JACKET'],
    'Mace': ['T6_MAIN_MACE', 'T5_MAIN_MACE', 'T4_MAIN_MACE'],
    'Nature Staff': ['T6_MAIN_NATURESTAFF', 'T5_MAIN_NATURESTAFF', 'T4_MAIN_NATURESTAFF'],
    'Ore': ['T6_ORE', 'T5_ORE', 'T4_ORE'],
    'Food': ['T6_MEAL_STEW', 'T5_MEAL_STEW', 'T4_MEAL_STEW'],
    'Gathering Gear': ['T6_GATHERER_HEAD', 'T5_GATHERER_HEAD', 'T4_GATHERER_HEAD'],
    'Shapeshifter Staff': ['T6_MAIN_SHAPESHIFTERSTAFF', 'T5_MAIN_SHAPESHIFTERSTAFF', 'T4_MAIN_SHAPESHIFTERSTAFF'],
    'Tool': ['T6_TOOL_PICKAXE', 'T5_TOOL_PICKAXE', 'T4_TOOL_PICKAXE'],
    'War Gloves': ['T6_MAIN_WARGLOVES', 'T5_MAIN_WARGLOVES', 'T4_MAIN_WARGLOVES'],
    'Bag': ['T6_BAG', 'T5_BAG', 'T4_BAG'],
    'Cape': ['T6_CAPE', 'T5_CAPE', 'T4_CAPE'],
    'Potion': ['T6_POTION_HEAL', 'T5_POTION_HEAL', 'T4_POTION_HEAL']
};
...
async function loadDailyCategoryIcon(category) {
    const aliases = {
        ...
        'Frost Staffs': 'Frost Staff',
        'Bag': 'Bag'
    };
    const rawKey = String(category || '').trim();
    const categoryKey = aliases[rawKey] || rawKey;
    if (dailyIconCache.has(categoryKey)) return dailyIconCache.get(categoryKey);

    // Prefer the actual Albion item render. This fixes missing category icons
    // such as Frost Staff / Bag and keeps the image tied to the real category.
    const itemIds = DAILY_CATEGORY_ITEM_IDS[categoryKey] || [];
    for (const itemId of itemIds) {
        const image = await loadDailyItemIcon(itemId);
        if (image) {
    ...
    // Legacy destiny icon fallback for the few categories that expose one.
...
function getDailyThemeForIndex(index) {
    return index === 0
        ? {
            accent: '#2da8ff',
            accent2: '#6fd1ff',
            glow: 'rgba(45,168,255,0.38)',
            panel: '#0b1828',
            panel2: '#102b46',
            gold: '#f5bd43'
        }
...
function drawCityBadge(ctx, city, x, y, size) {
    const cityStyles = {
        'Fort Sterling': { accent: '#9fdcff', fill: '#1a3347', mark: 'FS', glyph: '◆' },
        Thetford: { accent: '#d79cff', fill: '#352040', mark: 'TH', glyph: '✦' },
        Bridgewatch: { accent: '#f1bf64', fill: '#493018', mark: 'BW', glyph: '◆' },
        Lymhurst: { accent: '#98d89e', fill: '#1c3b29', mark: 'LY', glyph: '✦' },
        Martlock: { accent: '#c4cfdf', fill: '#303b4c', mark: 'MA', glyph: '◆' },
        Caerleon: { accent: '#e7a8a8', fill: '#3b2025', mark: 'CA', glyph: '♛' },
        Brecilien: { accent: '#8fd6e8', fill: '#183c4b', mark: 'BR', glyph: '◈' }
    };
    const style = cityStyles[city] || { accent: '#d2b273', fill: '#3d3022', mark: String(city || 'CT').slice(0, 2).toUpperCase(), glyph: '◆' };
    const cx = x + size / 2;
    const cy = y + size / 2;
    ctx.save();
    ...
    ctx.font = `900 ${Math.floor(size * 0.25)}px Arial, sans-serif`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(style.glyph, cx, cy - size * 0.10);
    ctx.fillStyle = '#f7f2e8';
    ctx.font = `900 ${Math.floor(size * 0.19)}px Arial, sans-serif`;
    ctx.fillText(style.mark, cx, cy + size * 0.19);
    ctx.restore();
}

function drawItemShowcase(ctx, image, category, x, y, size, theme) {
    const cx = x + size / 2;
    const cy = y + size / 2;
    drawGlowCircle(ctx, cx, cy, size * 0.40, theme.glow);
    ctx.save();
    ...
    if (image) {
        ...
        } catch (_) { drawCategoryPlaceholder(ctx, category, x + size * 0.18, y + size * 0.18, size * 0.64); }
    } else {
        drawCategoryPlaceholder(ctx, category, x + size * 0.18, y + size * 0.18, size * 0.64);
    }
    ctx.restore();
}

function drawMiniItemIcon(ctx, image, x, y, size, theme, label) {
    ctx.save();
    drawRoundRect(ctx, x, y, size, size, 9);
    ctx.fillStyle = '#09131d'; ctx.fill();
    ctx.strokeStyle = theme.accent; ctx.lineWidth = 1; ctx.stroke();
    if (image) {
        try { ctx.drawImage(image, x + 4, y + 4, size - 8, size - 8); } catch (_) {}
    }
    if (label) {
        ctx.fillStyle = 'rgba(4,8,12,0.78)';
        ctx.fillRect(x + 3, y + size - 16, size - 6, 13);
        ctx.fillStyle = '#f6efe4'; ctx.font = '700 8px Arial, sans-serif';
        ctx.textAlign = 'center'; ctx.fillText(label, x + size / 2, y + size - 6);
    }
    ctx.restore();
}
...
function drawDecorativeStars(ctx, width, height) {
    const points = [
        [30, 105, 2], [74, 42, 1], [190, 84, 1], [430, 48, 2], [610, 96, 1], [910, 102, 2],
        [935, 405, 1], [34, 500, 2], [478, 535, 1], [892, 548, 1], [110, 1070, 2], [840, 1075, 2]
    ];
    ctx.save();
...
function roundedHeaderBadge(ctx, x, y, w, h, fill, stroke, textColor, text) {
...
async function generateDailyBonusCard(serverChoice) {
    // Daily Bonus is intentionally Asia-only.
    serverChoice = 'asia';
    ...
    // Premium fantasy-game background.
    const bg = ctx.createLinearGradient(0, 0, width, height);
    bg.addColorStop(0, '#06111e');
    bg.addColorStop(0.52, '#091525');
    bg.addColorStop(1, '#040a12');
    ctx.fillStyle = bg; ctx.fillRect(0, 0, width, height);
    const glowA = ctx.createRadialGradient(250, 330, 20, 250, 330, 430);
    glowA.addColorStop(0, 'rgba(35,145,255,0.18)'); glowA.addColorStop(1, 'rgba(35,145,255,0)');
    ctx.fillStyle = glowA; ctx.fillRect(0, 0, width, height);
    const glowB = ctx.createRadialGradient(950, 360, 20, 950, 360, 430);
    glowB.addColorStop(0, 'rgba(255,180,40,0.16)'); glowB.addColorStop(1, 'rgba(255,180,40,0)');
    ctx.fillStyle = glowB; ctx.fillRect(0, 0, width, height);
    drawDecorativeStars(ctx, width, height);

    // Outer frame + inner frame.
    ctx.strokeStyle = '#b98a32'; ctx.lineWidth = 2;
    ctx.strokeRect(12, 12, width - 24, height - 24);
    ctx.strokeStyle = 'rgba(107,153,198,0.65)'; ctx.lineWidth = 1;
    ...
    ctx.textAlign = 'center';
    ctx.fillText('DAILY PRODUCTION BONUS', width / 2, 62);
    ctx.fillStyle = '#6fc6ff';
    ctx.font = '900 15px Arial, sans-serif';
    ctx.fillText('✦  ASIA (EAST)  ✦', width / 2, 88);

    // Reset pill.
    roundedHeaderBadge(ctx, width - 254, 35, 208, 40, '#2b2110', '#d69b2d', '#ffd66e', `NEXT RESET  ${nextReset.clock}`);
    ctx.fillStyle = '#7f8ea0'; ctx.font = '700 12px Arial, sans-serif';
    ctx.fillText(`BONUS TODAY   •   ${dateText}   •   ${statusText}`, width / 2, 116);

    const gap = 22;
    const cardY = 145;
    const cardW = (width - 72 - gap) / 2;
    const cardH = 590;
    const cardX = 36;
    const iconPromises = entries.map(entry => loadDailyCategoryIcon(entry.category));
    const iconImages = await Promise.all(iconPromises);

    entries.forEach((entry, index) => {
        const theme = getDailyThemeForIndex(index);
        ...
        ctx.strokeRect(x + 9, y + 9, cardW - 18, cardH - 18);

        ctx.fillStyle = theme.accent2;
        ctx.font = '800 12px Arial, sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(`TODAY  #${index + 1}`, x + 24, y + 30);
        ctx.fillStyle = '#f3eee7';
        ctx.font = '900 31px Arial, sans-serif';
        ctx.fillText(String(entry.category).toUpperCase(), x + 24, y + 68, cardW - 48);

        // Bonus ribbon: both percentage and multiplier are explicit.
        drawRoundRect(ctx, x + 24, y + 86, cardW - 48, 70, 12);
        ctx.fillStyle = theme.panel2; ctx.fill();
        ctx.strokeStyle = theme.accent; ctx.lineWidth = 1; ctx.stroke();
        ctx.fillStyle = '#9eafc1'; ctx.font = '700 10px Arial, sans-serif';
        ctx.fillText('CRAFTING BONUS', x + 40, y + 107);
        ctx.fillStyle = theme.gold; ctx.font = '900 31px Arial, sans-serif';
        ctx.fillText(`+${bonus}%`, x + 40, y + 139);
        ctx.fillStyle = '#eef5ff'; ctx.font = '900 24px Arial, sans-serif';
        ctx.textAlign = 'right'; ctx.fillText(`×${multiplier}`, x + cardW - 40, y + 137);
        ctx.textAlign = 'left';
        ctx.fillStyle = theme.accent2; ctx.font = '900 13px Arial, sans-serif';
        ctx.fillText('PRODUCTION MULTIPLIER', x + cardW - 196, y + 108);

        // Left: actual item icon.
        const iconBoxX = x + 24;
        const iconBoxY = y + 178;
        const iconBoxW = 210;
        const iconBoxH = 246;
        drawRoundRect(ctx, iconBoxX, iconBoxY, iconBoxW, iconBoxH, 14);
        ctx.fillStyle = '#07111b'; ctx.fill();
        ctx.strokeStyle = theme.accent; ctx.lineWidth = 1; ctx.stroke();
        ctx.fillStyle = '#8393a5'; ctx.font = '700 10px Arial, sans-serif';
        ctx.textAlign = 'center'; ctx.fillText('EXAMPLE ITEM', iconBoxX + iconBoxW / 2, iconBoxY + 23);
        drawItemShowcase(ctx, iconImages[index], entry.category, iconBoxX + 22, iconBoxY + 34, 166, theme);
        ctx.fillStyle = '#f0eadf'; ctx.font = '900 16px Arial, sans-serif';
        ctx.fillText(String(entry.category).toUpperCase(), iconBoxX + iconBoxW / 2, iconBoxY + 217);
        ctx.fillStyle = '#6f8092'; ctx.font = '700 9px Arial, sans-serif';
        ctx.fillText('REPRESENTATIVE ITEM', iconBoxX + iconBoxW / 2, iconBoxY + 235);

        // Right: city bonus + city icon.
        const infoX = x + 250;
        const infoY = y + 178;
        const infoW = cardW - 274;
        const infoH = 246;
        drawRoundRect(ctx, infoX, infoY, infoW, infoH, 14);
        ctx.fillStyle = '#0a1623'; ctx.fill();
        ctx.strokeStyle = 'rgba(125,160,193,0.34)'; ctx.lineWidth = 1; ctx.stroke();
        ctx.fillStyle = '#8fa0b2'; ctx.font = '700 10px Arial, sans-serif';
        ctx.textAlign = 'left'; ctx.fillText('BEST PLACE TO CRAFT', infoX + 18, infoY + 24);
        ctx.fillStyle = '#f3eee7'; ctx.font = '900 25px Arial, sans-serif';
        ctx.fillText(String(city).toUpperCase(), infoX + 18, infoY + 57, infoW - 86);
        ctx.fillStyle = theme.gold; ctx.font = '900 13px Arial, sans-serif';
        ctx.fillText(`CITY BONUS  +${Number(entry.baseBonus) || 0}%`, infoX + 18, infoY + 82);
        ctx.fillStyle = '#6e8092'; ctx.font = '700 10px Arial, sans-serif';
        ctx.fillText('BONUS CITY', infoX + 18, infoY + 118);
        ctx.fillStyle = '#d8e1ea'; ctx.font = '700 12px Arial, sans-serif';
        ctx.fillText('Craft here for the best local bonus', infoX + 18, infoY + 143, infoW - 105);
        // City icon is deliberately placed in the lower-right corner.
        drawCityBadge(ctx, city, infoX + infoW - 78, infoY + infoH - 84, 62);
        ctx.fillStyle = '#8092a5'; ctx.font = '700 9px Arial, sans-serif';
        ctx.textAlign = 'right'; ctx.fillText('BONUS CITY', infoX + infoW - 18, infoY + infoH - 14);

        // Bottom explanation strip.
        drawRoundRect(ctx, x + 24, y + 442, cardW - 48, 82, 12);
        ctx.fillStyle = '#06101a'; ctx.fill();
        ctx.strokeStyle = 'rgba(125,160,193,0.34)'; ctx.lineWidth = 1; ctx.stroke();
        ctx.fillStyle = theme.accent2; ctx.font = '800 10px Arial, sans-serif';
        ctx.fillText(`BONUS #${index + 1}   •   +${bonus}% PRODUCTION   •   ×${multiplier}`, x + 40, y + 467);
        ctx.fillStyle = '#d9e2ea'; ctx.font = '700 12px Arial, sans-serif';
        ctx.fillText(`${String(entry.category).toUpperCase()} receives today's production bonus`, x + 40, y + 491, cardW - 80);
        ctx.fillStyle = '#738598'; ctx.font = '700 9px Arial, sans-serif';
        ctx.fillText(`Crafting city: ${city}`, x + 40, y + 510);
    });

    // Bottom summary ribbon.
    drawRoundRect(ctx, 36, 754, width - 72, 70, 16);
    ctx.fillStyle = '#081522'; ctx.fill();
    ctx.strokeStyle = '#c09339'; ctx.lineWidth = 1; ctx.stroke();
    ctx.fillStyle = '#ffd467'; ctx.font = '900 12px Arial, sans-serif';
    ctx.textAlign = 'left'; ctx.fillText('BONUS TODAY', 58, 778);
    entries.forEach((entry, i) => {
        const sx = 58 + i * 350;
        ctx.fillStyle = '#f0ece5'; ctx.font = '900 17px Arial, sans-serif';
        ctx.fillText(`${String(entry.category).toUpperCase()}   +${Number(entry.dailyBonus) || 0}%`, sx, 801);
        if (i < entries.length - 1) {
            ctx.fillStyle = '#65788b'; ctx.font = '900 16px Arial, sans-serif'; ctx.fillText('•', sx + 285, 801);
        }
    });
    ctx.fillStyle = '#8091a2'; ctx.font = '700 10px Arial, sans-serif';
    ctx.textAlign = 'right'; ctx.fillText(`ASIA (EAST)  •  NEXT RESET ${nextReset.clock}  •  IN ${nextReset.countdown}`, width - 58, 778);
    ctx.fillStyle = '#f2e8d9'; ctx.font = '900 13px Arial, sans-serif';
    ctx.fillText('POWERED BY   Boss Bot', width - 58, 801);
