...
// สร้างรูป Battle Report สำหรับ /add guild และ /add player
// รูปแบบ: [Weapon Icon] Name | Kills | Deaths | Fame
async function generatePlayerStatsImage(players) {
    if (!players || !players.length) return null;
    ...
    const height = padding + headerHeight + (players.length * rowHeight) + totalHeight + padding;
    ...
    ctx.fillRect(0, 0, width, padding + headerHeight);

    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 22px sans-serif';
    ctx.fillText('Name', padding + 68, padding + 38);

    ctx.fillStyle = '#b99cff';
    ctx.font = 'bold 18px sans-serif';
    ctx.fillText('Kills', 505, padding + 38);

    ctx.fillStyle = '#ff6b6b';
    ctx.fillText('Deaths', 585, padding + 38);

    ctx.fillStyle = '#f4c95d';
    ctx.fillText('Fame', 680, padding + 38);
    ...
    // โหลดไอคอนอาวุธทุกคนพร้อมกัน
    const weaponImages = await Promise.all(players.map(async (p) => {
        const weaponId = normalizeAlbionItemId(p.weapon);
        if (!weaponId) return null;

        const baseId = weaponId.split('@')[0];
        const urls = [
            `https://render.albiononline.com/v1/item/${encodeURIComponent(weaponId)}.png`,
            `https://render.albiononline.com/v1/item/${encodeURIComponent(baseId)}.png`
        ];
    ...
    players.forEach((p, i) => {
        ...
        if (name.length > 27) name = `${name.slice(0, 24)}...`;

        ctx.fillStyle = '#4fc3f7';
        ctx.font = 'bold 19px sans-serif';
        ctx.fillText(name, padding + 68, y + 35);

        // Kills
        ctx.fillStyle = Number(p.kills) > 0 ? '#b99cff' : '#59616b';
        ctx.font = 'bold 18px sans-serif';
        ctx.fillText(String(Number(p.kills) || 0), 515, y + 35);

        // Deaths
        ctx.fillStyle = Number(p.deaths) > 0 ? '#ff6b6b' : '#59616b';
        ctx.fillText(String(Number(p.deaths) || 0), 605, y + 35);

        // Fame
        ctx.fillStyle = Number(p.fame) > 0 ? '#f4c95d' : '#59616b';
        ctx.fillText(formatFame(p.fame || 0), 680, y + 35);
    });

    // Total
    const totalY = padding + headerHeight + (players.length * rowHeight);

    ctx.strokeStyle = '#303844';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(padding, totalY);
    ctx.lineTo(width - padding, totalY);
    ctx.stroke();

    const totalKills = players.reduce((sum, p) => sum + (Number(p.kills) || 0), 0);
    const totalDeaths = players.reduce((sum, p) => sum + (Number(p.deaths) || 0), 0);
    const totalFame = players.reduce((sum, p) => sum + (Number(p.fame) || 0), 0);

    ctx.fillStyle = '#7f8a96';
    ctx.font = 'bold 16px sans-serif';
    ctx.fillText(`Total (${players.length})`, padding + 8, totalY + 38);

    ctx.fillStyle = '#b99cff';
    ctx.font = 'bold 18px sans-serif';
    ctx.fillText(String(totalKills), 515, totalY + 38);

    ctx.fillStyle = '#ff6b6b';
    ctx.fillText(String(totalDeaths), 605, totalY + 38);

    ctx.fillStyle = '#f4c95d';
    ctx.fillText(formatFame(totalFame), 680, totalY + 38);

    return new AttachmentBuilder(canvas.toBuffer('image/png'), {
        name: 'battle-report.png'
    });
}
...
async function buildBattleReportPayload(matchId, customTargetGuilds = []) {
    ...
    const allSortedRows = [...reportStats.values()]
        .sort((a, b) => b.fame - a.fame || b.kills - a.kills || b.damage - a.damage);

    const guildNamesList = customTargetGuilds.map(g => typeof g === 'string' ? g : g.name);

    // กรองตาม Guild / Player ที่ติดตาม
    ...
    // สร้างรูป Battle Report ตามตัวอย่าง:
    // [Weapon Icon] Name | Kills | Deaths | Fame
    const playerReportAttachment = await generatePlayerStatsImage(rows);
    ...
    if (executioner && executioner.kills > 0) {
        awardsText += `🎯 **Executioner:** ${executioner.displayName} (${executioner.kills} Kills)\n`;
    }
    if (feeder && feeder.deaths > 0) {
        awardsText += `💀 **Feeder:** ${feeder.displayName} (${feeder.deaths} Deaths)\n`;
    }

    const attachments = [];
    if (playerReportAttachment) attachments.push(playerReportAttachment);

    // Top Performance เดิม
    ...
    if (performancePlayers.length) {
        try {
            ...
            console.error('❌ Top performance image error:', err.message);
        }
    }
    ...
    return {
        ...
        guildTotalFrames,
        battleUrl,
        payload: { content: reportContent, files: attachments }
    };
}
            ...
            try {
                ...
                if (result.guildTotalFrames >= checkValue || result.totalFame >= checkValue) {
                    ...
                    if (channel) {
                        ...
                        // 2. ส่งรายงานการต่อสู้พร้อมรูปสรุป (รูปแบบเหมือนรูปที่ 2) ทันทีโดยไม่ต้องวางลิงก์เอง
