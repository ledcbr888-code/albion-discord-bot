...
// -------------------------------------------------------------
// 🟢 ระบบ OCR Timer + ลบข้อความอัตโนมัติหลังนับถอยหลังจบ 5 นาที
// -------------------------------------------------------------
async function handleImageTimer(message, imageUrl) {
    ...
    // เก็บอ้างอิงข้อความทั้งหมดที่บอทส่งออกไป เพื่อเตรียมลบทีเดียวหลังหมดเวลา
    const sentMessages = [];
    ...
    try {
        ...
        // ใช้ Regex กวาดหาตัวเลขและหน่วยเวลา (h, m, s) ทั่วทั้งข้อความ
        let h = 0, m = 0, s = 0;
        const timeRegex = /(\d+)\s*(h|m|s)/gi;
        let match;
        while ((match = timeRegex.exec(textClean)) !== null) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`ตรวจพบเวลาถอยหลัง **${h > 0 ? `${h} ชั่วโมง ` : ''}${m} นาที ${s} วินาที**\n\n🎯 **เวลาเปิดโดยประมาณ:** <t:${unlockTimestamp}:F> (<t:${unlockTimestamp}:R>)`)
        ...
        const timerInterval = setInterval(async () => {
            ...
            if (remainingMs <= 0) {
                ...
                // 🟢 เพิ่มคำสั่งตั้งเวลาลบข้อความทั้งหมดออกอัตโนมัติ หลังนับถอยหลังจบลง 5 นาที (300,000 มิลลิวินาที)[cite: 3]
                setTimeout(async () => {
