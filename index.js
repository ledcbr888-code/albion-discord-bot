...
let lastDailyReportDate = ''; // ป้องกันการส่งซ้ำในวันเดียวกัน
...
function loadData() {
    try {
        ...
        console.log(`📁 Tracking: ${targetGuilds.length} guilds, ${targetPlayers.length} players, ${autoBattleConfigs.length} auto-battle configs, ${dailyAutoConfigs.length} daily auto configs`);
    } catch (err) {
...
// ---------------------------
// HELPER FOR DAILY BONUS API
// ---------------------------
const SERVER_ENDPOINTS = {
    // Official Albion GameInfo hosts (the previous code used game-info-* which is invalid).
    west: [
        'https://gameinfo.albiononline.com/api/gameinfo/dailyactivity',
        'https://game-info.albiononline.com/api/gameinfo/dailyactivity'
    ],
    asia: [
        'https://gameinfo-sgp.albiononline.com/api/gameinfo/dailyactivity',
        'https://game-info-sgp.albiononline.com/api/gameinfo/dailyactivity'
    ],
    europe: [
        'https://gameinfo-ams.albiononline.com/api/gameinfo/dailyactivity',
        'https://game-info-ams.albiononline.com/api/gameinfo/dailyactivity'
    ]
};
...
async function fetchDailyBonus(serverKey) {
    const endpoints = SERVER_ENDPOINTS[serverKey] || SERVER_ENDPOINTS.asia;
    ...
    for (const url of urls) {
        try {
            ...
            if (res.data !== undefined && res.data !== null) {
                console.log(`✅ Daily bonus API connected: ${serverKey} -> ${url}`);
                return res.data;
            }
        } catch (err) {
            lastError = err;
            console.warn(`⚠️ Daily bonus endpoint failed (${serverKey}): ${url} -> ${err.message}`);
        }
    }

    console.error(`❌ Fetch daily bonus error (${serverKey}):`, lastError?.message || 'No response');
    return null;
}

async function generateDailyBonusEmbed(serverChoice) {
    const serverDisplayName = SERVER_NAMES[serverChoice] || 'Albion Server';
    const apiData = await fetchDailyBonus(serverChoice);

    if (!apiData) return null;

    // The GameInfo response can differ slightly by server/version. Normalize the
    // common shapes instead of assuming description/productionBonus are always top-level.
    const payload = Array.isArray(apiData) ? (apiData[0] || {}) : (apiData.data || apiData.activity || apiData);
    const item = Array.isArray(payload) ? (payload[0] || {}) : payload;
    const descriptionText = String(
        item.description ?? item.Description ?? item.details ?? item.Details ??
        item.text ?? item.Text ?? item.bonusDescription ?? item.BonusDescription ??
        'ไม่มีรายละเอียดเพิ่มเติมจากเซิร์ฟเวอร์'
    );
    const bonusType = String(
        item.productionBonus ?? item.ProductionBonus ?? item.bonusType ?? item.BonusType ??
        item.type ?? item.Type ?? item.activityType ?? item.ActivityType ??
        'โบนัสประจำวัน'
    );
    const resetValue = item.resetTime ?? item.ResetTime ?? item.nextReset ?? item.NextReset;
    let resetTimeFormatted = resetValue ? String(resetValue) : '10:00 UTC (17:00 น. ไทย)';

    return new EmbedBuilder()
        .setColor(0xF1C40F)
        .setTitle(`✨ **ALBION ONLINE — DAILY BONUS REPORT** ✨`)
        .setDescription(`📢 **โบนัสประจำวันอย่างเป็นทางการจากเซิร์ฟเวอร์**\n\`\`\`ansi\n\x1b[1;33m🌐 Server: ${serverDisplayName}\x1b[0m\n\`\`\``)
        .addFields(
            {
                name: '🎯 **หมวดหมู่โบนัสวันนี้ (DAILY BONUS)**',
                value: `>>> **\`${bonusType.toUpperCase()}\`**\n\n📌 *เพิ่มอัตรา Resource Return Rate (RRR) หรืออัตราผลผลิตประจำวัน*`,
                inline: false
            },
            {
                name: '📝 **รายละเอียดโบนัส (DETAILS)**',
                value: `\`\`\`text\n${descriptionText}\n\`\`\``,
                inline: false
            },
            {
                name: '⏰ **เวลาการรีเซ็ตวันถัดไป (DAILY RESET)**',
                value: `⏳ **${resetTimeFormatted}**\n*(ระบบจะเปลี่ยนโบนัสหมวดใหม่ทุกๆ 17:00 น. ตามเวลาไทย)*`,
                inline: false
            }
        )
        .setThumbnail('https://render.albiononline.com/v1/spell/T6_GVGSEASONREWARD_FAMEBUFF_SPELL.png')
        .setFooter({ text: 'Albion Online Live Data • Powered by BOTBOSS', iconURL: client.user.displayAvatarURL() })
        .setTimestamp();
}
...
async function checkAndSendDailyAutoReports() {
    ...
    if (utcHours === 10 && utcMinutes >= 5 && utcMinutes <= 10) {
        if (lastDailyReportDate === todayStr) return;

        console.log('⏰ Triggering automated daily bonus reports...');
        ...
        for (const config of dailyAutoConfigs) {
            try {
                ...
                const embed = await generateDailyBonusEmbed(config.serverChoice || 'asia');
                if (embed) {
                    await channel.send({
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'daily') {
        ...
        if (sub === 'get') {
            ...
            const [embedAsia, embedWest, embedEurope] = await Promise.all([
                generateDailyBonusEmbed('asia'),
                generateDailyBonusEmbed('west'),
                generateDailyBonusEmbed('europe')
            ]);

            const embeds = [embedAsia, embedWest, embedEurope].filter(Boolean);

            if (embeds.length === 0) {
                return interaction.editReply('❌ **เกิดข้อผิดพลาด:** ไม่สามารถเชื่อมต่อ Albion GameInfo API สำหรับ Daily Bonus ได้ในขณะนี้\n`ตรวจสอบ console log ของบอทเพื่อดู endpoint ที่ล้มเหลว`');
            }
            ...
            try {
                if (targetChannel1.id === interaction.channel.id) {
                    ...
                    await targetChannel1.send({ embeds: embeds });
        ...
        if (sub === 'setup') {
            ...
            return interaction.editReply(`✅ **ตั้งค่ารายงานโบนัสรายวันอัตโนมัติสำเร็จ!**\n- 🌐 เซิร์ฟเวอร์: **${SERVER_NAMES[serverChoice]}**\n- 📢 ห้องแจ้งเตือน: <#${channel.id}>\n- ⏰ เวลาแจ้งเตือน: **17:05 น. (เวลาไทย)** ของทุกวันครับ`);
        }
