...
async function generatePlayerWeaponReportImage(players, battleInfo = {}) {
    ...
    // 1. เรียงลำดับตาม Fame/Kills เพื่อทำ Ranking 01, 02, 03...
    const sortedPlayers = [...players].sort((a, b) => 
        (b.fame || 0) - (a.fame || 0) || (b.kills || 0) - (a.kills || 0)
    );

    // 2. คำนวณ Stats รวมสำหรับ Header ด้านบน
    ...
    // 3. กำหนดขนาด Canvas & Grid Layout (10 คนต่อแถวตามภาพตัวอย่าง)
    ...
    const gapY = 12;
    const padding = 24;
    const headerTopHeight = 70; // Albion Logo & Match Info
    const statsBarHeight = 75;  // 5 Stats Cards (Total Kills, Deaths, Fame, etc.)
    const gridOffsetY = padding + headerTopHeight + statsBarHeight + 20;

    const columns = Math.min(playersPerRow, sortedPlayers.length);
    const rows = Math.ceil(sortedPlayers.length / playersPerRow);

    const width = Math.max(1300, padding * 2 + columns * cardWidth + (columns - 1) * gapX);
    ...
    const bgGlow = ctx.createRadialGradient(width / 2, height / 3, 50, width / 2, height / 3, width / 1.5);
    bgGlow.addColorStop(0, '#151d28');
    bgGlow.addColorStop(1, '#0a0d12');
    ctx.fillStyle = bgGlow;
    ctx.fillRect(0, 0, width, height);

    // === 1. TOP HEADER (LOGO + MATCH INFO + PLAYERS BADGE) ===
    // Title Text
    ctx.fillStyle = '#e2e8f0';
    ctx.font = 'black 28px serif';
    ctx.fillText('BATTLE REPORT', padding + 10, padding + 32);

    // Sub-info
    ctx.fillStyle = '#64748b';
    ctx.font = 'bold 12px sans-serif';
    const serverTimeText = `EAST SERVER   |   ${formatUTCTime(battleInfo.battleTime || Date.now())}   |   MATCH ID: ${battleInfo.matchId || 'N/A'}`;
    ctx.fillText(serverTimeText, padding + 10, padding + 52);

    // Players Badge (ขวาล่าง Header)
    const badgeX = width - padding - 130;
    const badgeY = padding + 5;
    ctx.fillStyle = '#111827';
    ctx.beginPath();
    ctx.roundRect(badgeX, badgeY, 130, 52, 8);
    ctx.fill();
    ...
    ctx.fillText(`👥 ${sortedPlayers.length}`, badgeX + 35, badgeY + 28);
    ctx.fillStyle = '#9ca3af';
    ctx.font = 'bold 10px sans-serif';
    ctx.fillText('PLAYERS', badgeX + 37, badgeY + 44);

    // === 2. STATS BAR (5 CARDS) ===
    const statBoxWidth = (width - padding * 2 - 4 * 12) / 5;
    const statBoxY = padding + headerTopHeight;
    const statConfigs = [
        { label: 'TOTAL KILLS', val: totalKills.toLocaleString(), icon: '⚔️', color: '#a855f7' },
        { label: 'TOTAL DEATHS', val: totalDeaths.toLocaleString(), icon: '💀', color: '#ef4444' },
        { label: 'TOTAL FAME', val: formatFame(totalFame), icon: '👑', color: '#f59e0b' },
        { label: 'TOP KILLER', val: topKiller.name.slice(0, 10), sub: `${topKiller.kills} KILLS`, icon: '👾', color: '#c084fc' },
        { label: 'MVP (TOP FAME)', val: mvp.name.slice(0, 10), sub: `${formatFame(mvp.fame)} FAME`, icon: '🏆', color: '#fbbf24' }
    ];

    statConfigs.forEach((st, idx) => {
        const sx = padding + idx * (statBoxWidth + 12);
        
        // Card Background
        ctx.fillStyle = '#111827';
        ctx.beginPath();
        ctx.roundRect(sx, statBoxY, statBoxWidth, statsBarHeight - 10, 8);
        ctx.fill();
        ...
        ctx.fillText(st.icon, sx + 16, statBoxY + 38);

        // Label
        ctx.fillStyle = '#6b7280';
        ctx.font = 'bold 10px sans-serif';
        ctx.fillText(st.label, sx + 52, statBoxY + 22);

        // Value
        ctx.fillStyle = '#f9fafb';
        ctx.font = 'bold 18px sans-serif';
        ctx.fillText(String(st.val), sx + 52, statBoxY + 44);

        if (st.sub) {
            ctx.fillStyle = st.color;
            ctx.font = 'bold 10px sans-serif';
            ctx.fillText(st.sub, sx + 52, statBoxY + 58);
        }
    });
    ...
    sortedPlayers.forEach((p, i) => {
        ...
        ctx.lineWidth = 1;
        if (isMVP) {
            ctx.strokeStyle = '#f59e0b'; // Gold Border for MVP
            ctx.lineWidth = 1.8;
        } else if (isFeeder) {
            ctx.strokeStyle = '#991b1b'; // Red Border for Deaths
        } else {
        ...
        // Rank Badge Top-Left
        ctx.fillStyle = isMVP ? '#f59e0b' : '#374151';
        ctx.beginPath();
        ctx.roundRect(x + 5, y + 5, 22, 14, 3);
        ctx.fill();
        ctx.fillStyle = isMVP ? '#000000' : '#d1d5db';
        ctx.font = 'bold 9px sans-serif';
        ctx.fillText(rankNum, x + 8, y + 15);
        ...
        if (iconImages[i]) {
            try {
                ctx.drawImage(iconImages[i], imgX + 2, imgY + 2, imgSize - 4, imgSize - 4);
            } catch (_) {}
        }
        ...
        ctx.fillStyle = isMVP ? '#fbbf24' : '#e2e8f0';
        ctx.font = 'bold 11px sans-serif';
        const nameWidth = ctx.measureText(name).width;
        ctx.fillText(name, x + (cardWidth - nameWidth) / 2, y + 68);
        ...
        ctx.fillStyle = kVal > 0 ? '#c084fc' : '#9ca3af'; // Kills (Purple)
        ctx.fillText(kText, startStatX, y + 84);
        startStatX += ctx.measureText(kText).width;

        ctx.fillStyle = '#4b5563'; // Slash
        ctx.fillText(slashText, startStatX, y + 84);
        startStatX += ctx.measureText(slashText).width;

        ctx.fillStyle = dVal > 0 ? '#ef4444' : '#9ca3af'; // Deaths (Red)
        ctx.fillText(dText, startStatX, y + 84);

        // Fame Badge Bottom (Coin Icon + Fame Value)
        ctx.fillStyle = '#fbbf24';
        ctx.font = '10px sans-serif';
        const fameText = `🪙 ${formatFame(p.fame || 0)}`;
        const fameWidth = ctx.measureText(fameText).width;
        ctx.fillText(fameText, x + (cardWidth - fameWidth) / 2, y + 100);
    });
    ...
    ctx.font = 'bold 10px sans-serif';
    ctx.fillStyle = '#64748b';
    const poweredText = 'POWERED BY ALBION AUTO BATTLE BOT';
