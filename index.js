...
async function generateDailyBonusCard(serverChoice) {
    ...
    entries.forEach((entry, index) => {
        ...
        ctx.fillText(title, x + 112, y + 94);
        rounded(x + cardW - 128, y + 62, 88, 38, 19, '#4a2b0c', '#a66b20');
        ctx.fillStyle = '#f0b34c';
        ctx.font = 'bold 18px Arial, sans-serif';
        ctx.fillText(`+${entry.dailyBonus}%`, x + cardW - 113, y + 87);
        ...
        ctx.fillText(`${entry.city}  +${entry.baseBonus}%`, x + 174, y + 153);

        const iconData = iconSets[index] || { items: [], category: null };
        const icons = iconData.items || [];
        const iconSize = 54;
        const iconGap = 8;
        const startX = x + 32;
        const iconY = y + 190;
        ...
        ctx.fillStyle = '#867a70';
        ctx.font = 'bold 10px Arial, sans-serif';
        ctx.fillText('PRICE VS PRE-BONUS', x + 32, y + 305);
        ctx.fillText('VOLUME (7D)', x + 225, y + 305);
        ctx.fillStyle = '#d9d0c8';
        ctx.font = 'bold 15px Arial, sans-serif';
        ctx.fillText('N/A', x + 32, y + 329);
        ctx.fillText('N/A', x + 225, y + 329);
        ctx.fillStyle = '#6f9a6e';
        ctx.font = 'bold 11px Arial, sans-serif';
        ctx.fillText(' live data unavailable', x + 65, y + 329);
        ctx.fillText(' live data unavailable', x + 258, y + 329);
    });
