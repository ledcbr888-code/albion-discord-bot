...
let targetPlayers = [];
let targetGuilds = []; // รูปแบบใหม่เก็บเป็น Object เช่น { name: 'GuildName', channelId: '123456789' } หรือเก็บแบบระบุห้องแยกตามกิลด์
let targetKillTrackers = []; // สำหรับเก็บข้อมูลติดตามการฆ่า
let targetDeathTrackers = []; // สำหรับเก็บข้อมูลติดตามการตาย
...
function loadData() {
    try {
        ...
        // รองรับข้อมูลเก่าที่เป็น string array ให้แปลงเป็น object อัตโนมัติป้องกัน Error
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'add') {
        ...
        if (sub === 'guild') {
            ...
            if (existingIndex >= 0) {
                // อัปเดตห้องถ้ามีกิลด์นี้อยู่แล้ว
...
client.on('messageCreate', async message => {
    ...
    // ตรวจสอบว่าห้องที่ส่งข้อความมา ตรงกับห้องที่ตั้งค่าไว้ของกิลด์ใดๆ หรือไม่ (ถ้ามีกิลด์ที่ผูกห้องไว้)
    const matchingGuildConfig = targetGuilds.find(g => g.channelId === message.channel.id);
    
    // ถ้ามีการตั้งค่าห้องเฉพาะกิลด์ไว้ แต่ห้องที่ส่งข้อความมาไม่ตรงกับห้องที่กำหนด ให้ข้ามการดึงข้อมูลอัตโนมัติทันที
