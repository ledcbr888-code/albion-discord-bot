...
async function buildBattleReportPayload(matchId, customTargetGuilds = targetGuilds) {
    const apiPromise = /^\d+$/.test(matchId) ? fetchOfficialBattle(matchId) : Promise.resolve(null);
    const htmlPromise = fetchAlbionBBPage(matchId);
...
async function processBattleReport(input, targetContext, isMessage = false) {
    try {
        ...
        const { payload } = await buildBattleReportPayload(matchId);
...
async function checkAutoBattles() {
    ...
    try {
        ...
        $('a[href*="/battles/"]').each((_, el) => {
            ...
            const match = href.match(/\/battles\/(\d+)/);
            if (match && !recentMatches.includes(match[1])) {
                recentMatches.push(match[1]);
            }
        });
...
client.once('ready', async () => {
    ...
    try {
        ...
        $('a[href*="/battles/"]').each((_, el) => {
            const href = $(el).attr('href');
            const match = href.match(/\/battles\/(\d+)/);
            if (match) processedBattles.add(match[1]);
        });
        console.log(`✅ Initialized with ${processedBattles.size} existing matches. Monitoring for new battles...`);
    } catch (err) {
