...
async function generatePlayerWeaponReportImage(players, battleInfo = {}) {
    ...
    sortedPlayers.forEach((p, i) => {
        ...
        centerText(fameText, fameX + 70, fameY + 16, 11, '#fbbf24', '900');
    });
...
async function processBattleReport(input, targetContext, isMessage = false, customGuilds = null) {
    try {
        const matchId = extractMatchId(input);
        const { payload } = await buildBattleReportPayload(matchId, customGuilds || targetGuilds);
            ...
            try {
                ...
                if (result.guildTotalFrames >= checkValue || result.totalFame >= checkValue) {
                    ...
                    if (channel) {
                        ...
                        const fullContent = `${alertHeader}${result.payload.content}`;
                        ...
                        if (reportFiles.length > 0) {
                            ...
                            for (const file of reportFiles.slice(1)) {
...
client.on('messageCreate', async message => {
    ...
    try {
        ...
        const customGuilds = matchingGuildConfig ? [matchingGuildConfig] : null;
        await processBattleReport(match[0], status, true, customGuilds);
