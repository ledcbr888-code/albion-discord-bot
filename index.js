...
// 🟢 เพิ่มตัวแปรเก็บการตั้งค่า Image Timer
...
// -------------------------------------------------------------
// 🟢 ระบบ OCR Timer พร้อมรองรับกำหนดช่องต้นทางและช่องแจ้งเตือน
// -------------------------------------------------------------
async function handleImageTimer(message, imageUrl) {
    let statusMsg = null;
    // กำหนดช่องที่จะส่งข้อความแจ้งเตือน (ถ้าตั้งค่า alertChannelId ไว้ ให้ส่งไปห้องนั้น ถ้าไม่ได้ตั้งให้ส่งในห้องที่ส่งรูปมา)
    ...
    try {
        statusMsg = await message.reply('🔍 กำลังอ่านเวลาถอยหลังและวิเคราะห์รูปภาพ...');
        ...
        if (text.toLowerCase().includes('anomaly')) objectType = '🔮 Power Anomaly';
        else if (text.toLowerCase().includes('vortex')) objectType = '🌀 Power Vortex';
        else if (text.toLowerCase().includes('core')) objectType = '💎 Power Core';
        else if (text.toLowerCase().includes('chest')) objectType = '📦 Treasure Chest';

        for (const line of lines) {
            if (line.includes('m') || line.includes('s') || line.includes('h')) {
                const match = line.match(/(\d+)\s*h|(\d+)\s*m|(\d+)\s*s/gi);
                if (match) {
                    let h = 0, m = 0, s = 0;
                    match.forEach(mStr => {
                        if (mStr.toLowerCase().includes('h')) h = parseInt(mStr);
                        if (mStr.toLowerCase().includes('m')) m = parseInt(mStr);
                        if (mStr.toLowerCase().includes('s')) s = parseInt(mStr);
                    });
        ...
        if (totalSeconds <= 0) {
            await statusMsg.edit('⚠️ ไม่พบเวลาถอยหลัง (Unlocks in: XX m XX s) ในรูปภาพนี้');
            return;
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`ตรวจพบเวลาถอยหลัง **${Math.floor(totalSeconds / 60)} นาที ${totalSeconds % 60} วินาที**\n\n🎯 **เวลาเปิดโดยประมาณ:** <t:${unlockTimestamp}:F> (<t:${unlockTimestamp}:R>)`)
            .setFooter({ text: `บอทจะแจ้งเตือนนับถอยหลังทุกๆ 5 นาที (แจ้งเตือนที่ห้อง: #${alertChannel.name})` })
            .setTimestamp();

        await statusMsg.edit({ content: '✅ บันทึกเวลาถอยหลังเรียบร้อยแล้ว!', embeds: [embed] });
        ...
        const timerInterval = setInterval(async () => {
            ...
            if (remainingMs <= 0) {
                ...
                await alertChannel.send(`🚨 **เตือนด่วน!** ${objectType} **ปลดล็อก/เปิดแล้วตอนนี้!** 🚨`);
                return;
            }

            if (remainingMins <= 5) {
                await alertChannel.send(`⚠️ **เตือนด่วน!** ${objectType} จะเปิดในอีก **${remainingMins} นาที** (<t:${unlockTimestamp}:R>)`);
            } else {
                await alertChannel.send(`⏳ **อัปเดตเวลา:** ${objectType} เหลือเวลาอีกประมาณ **${remainingMins} นาที** (<t:${unlockTimestamp}:R>)`);
            }
...
const commands = [
    ...
    // 🟢 เพิ่มคำสั่ง /timer สำหรับตั้งค่าระบบจับเวลาภาพอัตโนมัติ
...
client.on('interactionCreate', async interaction => {
    ...
    // 🟢 จัดการคำสั่ง /timer สำหรับระบบตั้งค่าจับเวลาภาพ
...
// 🟢 ดักจับข้อความและรูปภาพ
client.on('messageCreate', async message => {
    if (message.author.bot) return;

    // 1. ตรวจจับรูปภาพสำหรับระบบ Timer (ถ้าเปิดใช้งาน)
    if (message.attachments.size > 0 && imageTimerConfig.enabled) {
        // เช็กเงื่อนไขห้องต้นทาง (ถ้ามีการตั้งค่า sourceChannelId ไว้ และไม่ใช่ห้องนั้น ให้ข้าม)
