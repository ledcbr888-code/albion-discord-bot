    ...
    if (utcHours === 10 && utcMinutes >= 5 && utcMinutes <= 10) {
        ...
        for (const config of dailyAutoConfigs) {
            try {
                ...
                if (embed) {
                    await channel.send({
                        content: `📢 **รายงานโบนัสรายวันอัตโนมัติประจําวันที่ ${todayStr}**`,
                        embeds: [embed]
                    });
                }
            } catch (err) {
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'daily') {
        ...
        if (sub === 'get') {
            ...
            const embed = await generateDailyBonusEmbed(serverChoice);

            if (!embed) {
                return interaction.editReply(`❌ **เกิดข้อผิดพลาด:** ไม่สามารถเชื่อมต่อ API ดึงข้อมูลโบนัสรายวันของเซิร์ฟเวอร์ **${SERVER_NAMES[serverChoice]}** ได้ในขณะนี้`);
            }
            ...
            try {
                if (targetChannel.id === interaction.channel.id) {
                    ...
                    await interaction.editReply(`✅ ส่งรายงานโบนัสประจำวันไปยังห้อง <#${targetChannel.id}> เรียบร้อยแล้ว!`);
                }
            } catch (err) {
                console.error('❌ Daily command response error:', err.message);
                await interaction.editReply(`❌ ไม่สามารถส่งข้อความไปยังห้อง <#${targetChannel.id}> ได้ โปรดตรวจสอบสิทธิ์การส่งข้อความของบอท`);
            }
        }
