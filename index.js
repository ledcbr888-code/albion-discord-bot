...
async function fetchAlbionBBWeaponMap(matchId) {
    // AlbionBB's Asia endpoint keeps MainHand in the compact kill payload.
    // Use it as a fallback because the official /battles/{id} player snapshot
    // may contain stats without Equipment.
    const urls = [
        ...
        try {
            ...
            const addEntity = (entity) => {
                ...
                for (const c of candidates) {
                    ...
                    if (!id) continue;
                    // Trust MainHand/Weapon fields from AlbionBB; do not reject
                    // an unfamiliar future weapon ID just because our keyword list
...
async function fetchOfficialBattleWeaponMap(matchId) {
    // The /battles/{id} snapshot often has no Equipment for every player.
    // Official battle events contain the Victim's full Equipment and usually
    // GroupMembers as well, so use them to fill the missing MainHand icons.
    const baseUrls = getApiUrls(matchId).map(url => url.replace(/\/battles\//i, '/events/battle/'));
    const map = new Map();
    const maxPages = 12; // prevents an endless poll on unusually large battles
...
function extractMainHandInfo(obj) {
    ...
    for (const candidate of candidates) {
        ...
        // MainHand/Weapon is already a slot-qualified field. Do not require
        // the ID to match our keyword list, because new/legacy Albion weapon
        // IDs can exist without being covered by isWeaponItemId().
...
// ฟังก์ชันสร้างรูปตารางสรุปกิลด์ตามรายการที่ caller เลือก
async function generateGuildSummaryImage(guildsData) {
    if (!guildsData || !guildsData.length) return null;
    
    // Caller decides which guilds should be displayed (for Auto-Battle:
    // tracked guild + Top 3 other guilds).
    ...
    // พื้นหลังการ์ด/รูปภาพ Discord Style
    ...
    // วาดข้อมูลแต่ละกิลด์ (เฉพาะ Top 5)
...
async function generatePlayerWeaponReportImage(players) {
    ...
    const padding = 18;
    const height = headerHeight + players.length * rowHeight + footerHeight + padding;
    const canvas = createCanvas(width, height);
    const ctx = canvas.getContext('2d');

    // Discord-like dark report background
    ...
    ctx.font = 'bold 20px sans-serif';
    ctx.fillText('Name', 86, 39);
    ctx.fillStyle = '#b58cff';
    ctx.fillText('Kills', 505, 39);
    ctx.fillStyle = '#ff5b64';
    ctx.fillText('Deaths', 585, 39);
    ctx.fillStyle = '#f5c84c';
    ctx.fillText('Fame', 680, 39);

    ctx.strokeStyle = '#252b33';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(18, headerHeight);
    ctx.lineTo(width - 18, headerHeight);
    ctx.stroke();

    // Load all icons in parallel. Use the official Render service with an
    // axios buffer instead of loadImage(URL), which is more reliable on hosts.
    ...
    players.forEach((p, i) => {
        ...
        // Icon frame - still visible when an API entry has no weapon.
        ctx.fillStyle = '#202833';
        ctx.beginPath();
        ctx.roundRect(iconX, iconY, iconSize, iconSize, 7);
        ctx.fill();

        if (iconImages[i]) {
            try {
                ctx.drawImage(iconImages[i], iconX, iconY, iconSize, iconSize);
            } catch (_) {}
        }

        let name = String(p.displayName || p.name || 'Unknown');
        if (name.length > 26) name = `${name.slice(0, 23)}...`;
        ctx.fillStyle = '#25b9ff';
        ctx.font = 'bold 18px sans-serif';
        ctx.fillText(name, 86, y + 35);

        ctx.font = 'bold 18px monospace';
        ctx.textAlign = 'right';
        ctx.fillStyle = '#b58cff';
        ctx.fillText(String(p.kills || 0), 535, y + 35);
        ctx.fillStyle = p.deaths > 0 ? '#ff5b64' : '#68717d';
        ctx.fillText(String(p.deaths || 0), 625, y + 35);
        ctx.fillStyle = '#f5c84c';
        ctx.fillText(formatFame(p.fame || 0), 742, y + 35);
        ctx.textAlign = 'left';

        ctx.strokeStyle = '#171d24';
        ctx.beginPath();
        ctx.moveTo(18, y + rowHeight);
        ctx.lineTo(width - 18, y + rowHeight);
        ctx.stroke();
    });

    const totalKills = players.reduce((n, p) => n + (Number(p.kills) || 0), 0);
    const totalDeaths = players.reduce((n, p) => n + (Number(p.deaths) || 0), 0);
    const totalFame = players.reduce((n, p) => n + (Number(p.fame) || 0), 0);
    const footerY = headerHeight + players.length * rowHeight + 20;

    ctx.fillStyle = '#7d8793';
    ctx.font = 'bold 16px monospace';
    ctx.fillText(`Total (${players.length})`, 24, footerY + 22);
    ctx.textAlign = 'right';
    ctx.fillStyle = '#b58cff';
    ctx.fillText(String(totalKills), 535, footerY + 22);
    ctx.fillStyle = totalDeaths > 0 ? '#ff5b64' : '#68717d';
    ctx.fillText(String(totalDeaths), 625, footerY + 22);
    ctx.fillStyle = '#f5c84c';
    ctx.fillText(formatFame(totalFame), 742, footerY + 22);
    ctx.textAlign = 'left';
...
async function buildBattleReportPayload(matchId, customTargetGuilds = [], options = {}) {
    ...
    // Weapon enrichment has 2 fallbacks:
    // 1) Official battle events -> full Victim/GroupMembers Equipment.MainHand
    // 2) AlbionBB compact kills -> MainHand for kill participants
    try {
    ...
    const rows = allSortedRows.filter(p => {
        ...
        // Exact match only: "DAMN" must NOT match "damn bor".
    ...
    // Main player report requested for /add guild and /add player.
    try {
    ...
    // สร้างรูปภาพสรุปกิลด์ (Guild Summary Image)
    let guildSummaryAttachment = null;
    try {
        if (apiData.guilds) {
            let guildsData = Object.values(apiData.guilds).map(g => ({
                name: g.name || 'Unknown',
                playersCount: g.memberCount || g.players?.length || 0,
                kills: g.kills || 0,
                deaths: g.deaths || 0,
            ...
            if (options.autoBattle && guildNamesList.length > 0) {
                // Auto-Battle: always include the tracked Guild + Top 3 OTHER
                // guilds from this battle. The tracked guild is never displaced.
                ...
                // Normal manual report: keep the previous exact tracked-guild behavior.
    ...
    if (guildSummaryAttachment) {
        attachments.push(guildSummaryAttachment); // player battle report remains the first image
    }
...
async function processBattleReport(input, targetContext, isMessage = false) {
    try {
        ...
        // Discord will place multiple image attachments side-by-side when they fit.
        // Send each image as a separate message so they are always stacked vertically.
        if (files.length > 1) {
            const firstPayload = {
...
async function checkAutoBattles() {
    ...
    try {
        for (const config of autoBattleConfigs) {
            ...
            try {
                // Verify the Official Albion Guild name before creating an Auto-Battle report.
                // AlbionBB search can return partial matches, e.g. "DAMN" -> "damn bor".
                ...
                if (result.guildTotalFrames >= checkValue || result.totalFame >= checkValue) {
                    ...
                    if (channel) {
                        // 1. ส่งข้อความแจ้งเตือน (รูปแบบเหมือนรูปที่ 1)
                        await channel.send({
                            content:
                                `[🚨](https://discord.com/assets/8d433206d7a0fe81.svg) **Auto-Battle Alert!** ตรวจพบไฟต์ใหม่ของกิลด์ **${config.targetGuild}**\n` +
                                `[⚔️](https://discord.com/assets/fa2c28d64be33d41.svg) รวม Kills + Deaths: \`${result.guildTotalFrames.toLocaleString()}\`\n` +
                                `[💰](https://discord.com/assets/60e4658040396168.svg) Total Fame: \`${formatFame(result.totalFame)}\`\n` +
                                `[🔗](https://discord.com/assets/ed3a958a3bb08d4a.svg) **Battle Link:** <${result.battleUrl}>`
                        });

                        // 2. ส่งรายงานเป็นรูปแยกข้อความ เพื่อบังคับให้รูปเรียงบน-ล่าง
                        // แทนการส่งหลาย attachments ในข้อความเดียวที่ Discord อาจจัดวางซ้าย-ขวา
...
const commands = [
    ...
    new SlashCommandBuilder().setName('autobattle').setDescription('จัดการระบบติดตามไฟต์อัตโนมัติ')
        ...
        .addSubcommand(s => s.list = s.setName('list').setDescription('แสดงรายชื่อกิลด์ที่ตั้งค่า Auto-Battle ในเซิร์ฟเวอร์นี้'))
