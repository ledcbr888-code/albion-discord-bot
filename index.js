...
async function processBattleReport(input, targetContext, isMessage = false) {
    try {
        ...
        // 🟢 [แก้ไขตรงนี้] กรองเฉพาะผู้เล่นที่อยู่ใน targetPlayers หรือสังกัด targetGuilds เท่านั้น
        const rows = allSortedRows.filter(p => {
            // หากไม่มีการตั้งค่าติดตามเลย จะไม่ดึงข้อมูลใดๆ มาแสดง
        ...
        // คำนวณยอดรวม TOTAL เฉพาะกลุ่มผู้เล่นที่ถูกกรองนำมาแสดง
        ...
        // ส่วนเนื้อหาผู้เล่น (คำนวณและตัดข้อความเมื่อใกล้เกิน 2,000 ตัวอักษร)
        let body = '';
        if (rows.length === 0) {
        ...
        // รูป Top Performance จะดึงเฉพาะผู้เล่นที่ผ่านการกรองเช่นเดียวกัน
