...
async function fetchRecentOfficialBattles() {
    const urls = [
        'https://gameinfo.albiononline.com/api/gameinfo/battles?sort=recent',
        'https://gameinfo-sgp.albiononline.com/api/gameinfo/battles?sort=recent'
    ];
    for (const url of urls) {
        try {
            const response = await axios.get(url, {
                timeout: 30000,
                headers: { 'User-Agent': 'Albion-Discord-Bot/1.0', 'Accept': 'application/json' }
            });
            const data = response.data;
            const list = Array.isArray(data) ? data : Array.isArray(data?.battles) ? data.battles : Array.isArray(data?.data) ? data.data : [];
            const ids = list.map(b => b?.id ?? b?.Id ?? b?.battleId ?? b?.BattleId).map(id => String(id || '').trim()).filter(id => /^\d+$/.test(id));
...
async function buildBattleReportPayload(matchId, customTargetGuilds = targetGuilds) {
    ...
    let guildTotalFrames = 0; // Kills + Deaths รวมของกิลด์ที่กำลังตรวจสอบ
    ...
    for (const p of targetRowsToCalc) {
        ...
        // คำนวณ Kills + Deaths เฉพาะผู้เล่นในกิลด์เป้าหมาย (กรณีเช็ก Auto-Battle)
        if (customTargetGuilds.length > 0 && p.guild) {
            const isGuildMatch = customTargetGuilds.some(g => p.guild.trim().toLowerCase().includes(g.trim().toLowerCase()) || g.trim().toLowerCase().includes(p.guild.trim().toLowerCase()));
            if (isGuildMatch) {
    ...
    return {
        matchId,
        totalFame,
        guildTotalFrames, // ส่งค่า Kills + Deaths รวมออกไปเช็กเงื่อนไข
        battleUrl,
...
async function checkAutoBattles() {
    ...
    try {
        ...
        for (const matchId of recentMatches.slice(0, 10)) {
            ...
            for (const config of autoBattleConfigs) {
                try {
                    ...
                    // เปลี่ยนเงื่อนไขจาก minFame เป็น guildTotalFrames (Kills + Deaths ขั้นต่ำ)
                    if (result.guildTotalFrames >= config.minFrames) {
            ...
            if (processedBattles.size > 100) {
                const arr = [...processedBattles];
                processedBattles = new Set(arr.slice(50));
            }
        }
    } catch (err) {
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'autobattle') {
        ...
        if (sub === 'set') {
            ...
            const minFrames = parseFameValue(minFramesInput); // ใช้ฟังก์ชันแปลงตัวเลข/ตัวอักษรย่อร่วมกันได้
