...
function drawCityBadge(ctx, city, x, y, size) {
    const cityStyles = {
        'Fort Sterling': { fill: '#34566a', accent: '#a9d7e8', mark: 'FS' },
        Thetford: { fill: '#5a3128', accent: '#e2a06d', mark: 'TH' },
        Bridgewatch: { fill: '#77502d', accent: '#f0c06a', mark: 'BW' },
        Lymhurst: { fill: '#315640', accent: '#9bd29f', mark: 'LY' },
        Martlock: { fill: '#4b5365', accent: '#bfc8dd', mark: 'MA' },
        Caerleon: { fill: '#583a57', accent: '#d7a8df', mark: 'CA' },
        Brecilien: { fill: '#3d5361', accent: '#a8d8db', mark: 'BR' }
    };
    const style = cityStyles[city] || { fill: '#4b4037', accent: '#d2b273', mark: String(city || 'CT').slice(0, 2).toUpperCase() };
    ctx.save();
...
async function generateDailyBonusCard(serverChoice) {
    ...
    const iconTasks = entries.map(async entry => ({
        items: await Promise.all((DAILY_CATEGORY_ITEM_IDS[entry.category] || []).map(loadDailyItemIcon)),
    ...
    entries.forEach((entry, index) => {
        ...
        const border = entry.dailyBonus >= 20 ? '#b36a1e' : '#553a20';
        rounded(x, y, cardW, cardH, 14, '#17120f', border);
        ctx.fillStyle = '#927e70';
        ctx.font = 'bold 12px Arial, sans-serif';
        ctx.fillText('PRODUCTION BONUS', x + 30, y + 39);

        const title = String(entry.category).toUpperCase();
        ctx.fillStyle = '#f2eee9';
        ctx.font = '900 34px Arial, sans-serif';
        ctx.fillText(title, x + 112, y + 94, cardW - 290);
        rounded(x + cardW - 142, y + 64, 100, 36, 18, '#4a2b0c', '#a66b20');
        ctx.fillStyle = '#f0b34c';
        ctx.font = 'bold 17px Arial, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(`+${entry.dailyBonus}%`, x + cardW - 92, y + 87);
        ctx.textAlign = 'left';

        ctx.fillStyle = '#33251c';
        ctx.fillRect(x + 30, y + 123, cardW - 60, 1);
        ctx.fillStyle = '#8e8177';
        ctx.font = 'bold 11px Arial, sans-serif';
        ctx.fillText('BEST CRAFT CITY', x + 30, y + 153);
        ctx.fillStyle = '#eee7df';
        ctx.font = 'bold 16px Arial, sans-serif';
        ctx.fillText(`${entry.city}  +${entry.baseBonus}%`, x + 174, y + 153, cardW - 260);

        const iconData = iconSets[index] || { items: [], category: null };
        const loadedItemIcons = (iconData.items || []).filter(Boolean);
        // Never repeat the category icon across every slot. If the individual
        // item endpoint is unavailable, show one category icon only.
        const icons = loadedItemIcons.length ? loadedItemIcons : (iconData.category ? [iconData.category] : []);
        const iconSize = 54;
        const iconGap = 8;
        const startX = x + 32;
        const iconY = y + 190;
        icons.forEach((image, iconIndex) => {
            const ix = startX + iconIndex * (iconSize + iconGap);
            rounded(ix, iconY, iconSize, iconSize, 7, '#29231e', '#4f463d');
            try { ctx.drawImage(image, ix + 3, iconY + 3, iconSize - 6, iconSize - 6); } catch (_) {}
        });

        // Always show a category icon even when individual item render URLs fail.
        if (iconData.category) {
            try { ctx.drawImage(iconData.category, x + 42, y + 62, 58, 58); } catch (_) {}
        }

        // Decorative crafting route panel uses a city badge at bottom-right.
        ctx.fillStyle = '#8e8177';
        ctx.font = 'bold 10px Arial, sans-serif';
        ctx.fillText('CRAFTING ROUTE', x + 32, y + 337);
        ctx.fillStyle = '#c2a36a';
        ctx.font = 'bold 13px Arial, sans-serif';
        ctx.fillText('BONUS CITY VERIFIED', x + 32, y + 358);
        ctx.strokeStyle = '#46352a';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(x + 32, y + 373);
        ctx.lineTo(x + cardW - 150, y + 373);
        ctx.stroke();
    ...
    const footer = dailyData.stale
        ...
        : `${dailyData.source || 'AO-SAGE'} — CONFIRMED ${String(dailyData.date || '').slice(0, 10)}`;
    ctx.fillStyle = '#6c5f55';
    ctx.font = 'bold 11px Arial, sans-serif';
    ctx.fillText(footer, 44, height - 18);
    ...
    for (const config of dailyAutoConfigs) {
        ...
        try {
            ...
            if (card) {
                ...
                await channel.send({
                    content:
                        `⚠️ **รายงานโบนัสประจำวันที่ ${todayStr}**\n` +
                        `ไม่พบข้อมูล Daily Bonus ของเซิร์ฟเวอร์ **${SERVER_NAMES[serverKey]}** ` +
                        `จาก AO-SAGE, AODP และ cache ล่าสุดในขณะนี้`
                });
            }
