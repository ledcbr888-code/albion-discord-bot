...
async function generatePlayerWeaponReportImage(players, battleInfo = {}) {
    ...
    drawRoundRect(ctx, badgeX, badgeY, 140, 52, 10);
    ctx.fill();
    ...
    ctx.fillText(`${sortedPlayers.length}`, badgeX + 24, badgeY + 28);
    ctx.fillStyle = '#94a3b8';
    ctx.font = 'bold 10px sans-serif';
    ctx.fillText('PLAYERS', badgeX + 58, badgeY + 28);

    const statBoxWidth = (width - padding * 2 - 4 * 14) / 5;
    const statBoxY = padding + headerTopHeight;
    const statConfigs = [
        { label: 'TOTAL KILLS', val: totalKills.toLocaleString(), color: '#c084fc' },
        { label: 'TOTAL DEATHS', val: totalDeaths.toLocaleString(), color: '#f87171' },
        { label: 'TOTAL FAME', val: formatFame(totalFame), color: '#fbbf24' },
        { label: 'TOP KILLER', val: topKiller.name.slice(0, 11), sub: `${topKiller.kills} KILLS`, color: '#e879f9' },
        { label: 'MVP (TOP FAME)', val: mvp.name.slice(0, 11), sub: `${formatFame(mvp.fame)} FAME`, color: '#facc15' }
    ];
    ...
    statConfigs.forEach((st, idx) => {
        ...
        ctx.fillStyle = '#0f172a';
        drawRoundRect(ctx, sx, statBoxY, statBoxWidth, statsBarHeight - 10, 10);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(sx + 24, statBoxY + 36, 6, 0, Math.PI * 2);
        ctx.fill();

        ctx.fillStyle = '#64748b';
        ctx.font = 'bold 11px sans-serif';
        ctx.fillText(st.label, sx + 40, statBoxY + 24);

        ctx.fillStyle = '#f8fafc';
        ctx.font = 'bold 18px sans-serif';
        ctx.fillText(String(st.val), sx + 40, statBoxY + 46);

        if (st.sub) {
            ctx.fillStyle = st.color;
            ctx.font = 'bold 11px sans-serif';
            ctx.fillText(st.sub, sx + 40, statBoxY + 62);
        }
    });
    ...
    sortedPlayers.forEach((p, i) => {
        ...
        drawRoundRect(ctx, x + 6, y + 6, 26, 16, 4);
        ctx.fill();
        ...
        ctx.fillText(kText, startStatX, y + 114);
        startStatX += ctx.measureText(kText).width;

        ctx.fillStyle = '#475569'; 
        ctx.fillText(slashText, startStatX, y + 114);
        startStatX += ctx.measureText(slashText).width;

        ctx.fillStyle = dVal > 0 ? '#ef4444' : '#94a3b8'; 
        ctx.fillText(dText, startStatX, y + 114);

        const fameText = formatFame(p.fame || 0);
        ctx.font = 'bold 12px sans-serif';
        const textWidth = ctx.measureText(fameText).width;
        
        // ขยายขนาดไอคอน Fame ให้ใหญ่ขึ้น
        const fameIconSize = 22;
        const gapBetween = 4;
        const totalFameBadgeWidth = fameImg ? (fameIconSize + gapBetween + textWidth) : textWidth;
        const fameStartX = x + (cardWidth - totalFameBadgeWidth) / 2;

        if (fameImg) {
            try {
                ctx.drawImage(fameImg, fameStartX, y + 122, fameIconSize, fameIconSize);
            } catch (_) {}
            ctx.fillStyle = '#fbbf24';
            ctx.fillText(fameText, fameStartX + fameIconSize + gapBetween, y + 138);
        } else {
            ctx.fillStyle = '#fbbf24';
            ctx.fillText(fameText, fameStartX, y + 138);
        }
    });
