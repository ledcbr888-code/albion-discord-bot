...
// 🔧 อัปเดตฟังก์ชัน handleImageTimer ที่รวมการแต่งภาพ (Sharp) + Whitelist + Text Sanitization ไว้ด้วยกัน
async function handleImageTimer(message, imageUrl) {
    ...
    try {
        ...
        // 🔍 2. สแกนด้วย Tesseract พร้อมกำหนด Whitelist (บังคับอ่านเฉพาะตัวอักษรที่เกี่ยวข้อง)
        const { data: { text } } = await Tesseract.recognize(processedBuffer, 'eng', {
            tessedit_char_whitelist: '0123456789hms: '
        });
        ...
        const timeRegexLetter = /(\d+)\s*([hms])/gi;
        let match;
