...
async function processBattleReport(input, targetContext, isMessage = false) {
    try {
        ...
        // Always finalize the original status message first. This prevents the
        // "กำลังดึงสถิติ..." message from disappearing when attachment upload fails.
        const finalText = payload.content || '⚠️ สร้างรายงานได้ แต่ไม่มีข้อความสรุป';
        if (isMessage) {
            await targetContext.edit({ content: finalText }).catch(async err => {
                console.error('❌ Could not edit status message:', err.message);
            });
            for (const file of files) {
                try {
                    await targetContext.channel.send({ files: [file] });
                } catch (err) {
                    console.error('❌ Could not send report image:', err.message);
                    await targetContext.channel.send({ content: `⚠️ สร้างรายงานแล้ว แต่ส่งรูปไม่ได้: ${err.message}` }).catch(() => {});
                }
            }
        } else {
            await targetContext.editReply({ content: finalText }).catch(async err => {
                console.error('❌ Could not edit interaction reply:', err.message);
            });
            for (const file of files) {
                try {
