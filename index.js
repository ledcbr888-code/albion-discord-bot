...
async function generateTopPerformanceImage(players) {
    ...
    for (let i = 0; i < players.length; i++) {
        ...
        if (barWidth > 0) {
            ...
            ctx.beginPath();
            const radius = Math.min(12, barWidth / 2, cardHeight / 2);
            ctx.roundRect(cardX, y, barWidth, cardHeight, radius);
            ctx.fill();
        }
        if (weaponImages[i]) {
            const size = 60;
...
// 🌐 ดึงสถิติไฟต์ล่าสุดจาก Official Albion Online API โดยตรง (เสถียร ไม่ติด Cloudflare)
async function fetchRecentBattlesFromOfficialApi() {
    const apiEndpoints = [
        'https://gameinfo-sgp.albiononline.com/api/gameinfo/battles?limit=50&offset=0&sort=recent',
        'https://gameinfo.albiononline.com/api/gameinfo/battles?limit=50&offset=0&sort=recent'
    ];
    ...
    for (const url of apiEndpoints) {
        try {
            ...
            console.warn(`⚠️ Fetch official battle list error (${url}):`, err.message);
        }
    }
    return [];
}
...
async function checkAutoBattles() {
    ...
    try {
        const officialRecentBattles = await fetchRecentBattlesFromOfficialApi();

        for (const config of autoBattleConfigs) {
            let matchIdsToTest = [];

            if (officialRecentBattles.length > 0) {
                const matching = officialRecentBattles.filter(b => {
                    if (!b || !b.guilds) return false;
                    const guildNames = Object.values(b.guilds).map(g => g?.name).filter(Boolean);
                    return isExactGuildMatch(config.targetGuild, guildNames);
                });
                matchIdsToTest = matching.map(b => String(b.id));
            }

            if (!matchIdsToTest.length) {
                const scrapedMatches = await fetchGuildRecentBattles(config.targetGuild);
                matchIdsToTest = scrapedMatches;
            }

            for (const latestMatchId of matchIdsToTest) {
        ...
        if (processedBattles.size > 500) {
            const arr = [...processedBattles];
            processedBattles = new Set(arr.slice(250));
        }
    } catch (err) {
