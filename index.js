...
const commands = [
    ...
    // อัปเดต /kill ให้มี Subcommand (set, list, remove)
    ...
    // อัปเดต /death ให้มี Subcommand (set, list, remove)
...
client.on('interactionCreate', async interaction => {
    ...
    // จัดการคำสั่ง /kill (set, list, remove)
    if (commandName === 'kill') {
        const sub = interaction.options.getSubcommand();
        ...
        if (sub === 'set') {
            ...
            // ป้องกันการเพิ่มซ้ำประเภทและชื่อเดิมในห้องเดิม (หรืออัปเดตห้อง)
    ...
    // จัดการคำสั่ง /death (set, list, remove)
    if (commandName === 'death') {
        ...
        if (sub === 'list') {
            ...
            const listText = serverTrackers.map((t, i) => `${i + 1}. [**${t.type.toUpperCase()}**] **${name || t.name}** | ห้อง: <#${t.channelId}>`).join('\n');
            // แก้ไขจุดแสดงผลชื่อให้ถูกต้อง
