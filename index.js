...
// ฟังก์ชันดึง Event ล่าสุด (Kill/Death) จาก Albion Online API ทางการ
async function fetchRecentEvents() {
    try {
        const urls = [
            'https://gameinfo.albiononline.com/api/gameinfo/events?limit=50&offset=0',
            'https://gameinfo-sgp.albiononline.com/api/gameinfo/events?limit=50&offset=0'
        ];
        for (const url of urls) {
            try {
        ...
        console.warn('⚠️ Fetch events error:', err.message);
    }
    return [];
}
...
async function checkKillDeathTrackers() {
    ...
    for (const ev of events) {
        ...
        const killerName = String(killer.Name || '').trim();
        const killerGuild = String(killer.GuildName || '').trim();
        const victimName = String(victim.Name || '').trim();
        const victimGuild = String(victim.GuildName || '').trim();

        // 1. ตรวจสอบ Kill Trackers
        for (const tracker of targetKillTrackers) {
            const isMatch = tracker.type === 'player' 
                ? killerName.toLowerCase() === tracker.name.toLowerCase()
                : killerGuild.toLowerCase() === tracker.name.toLowerCase();

            if (isMatch) {
                const channel = await client.channels.fetch(tracker.channelId).catch(() => null);
                if (channel) {
                    const fame = Number(ev.TotalKillFame || 0).toLocaleString();
                    const embed = new EmbedBuilder()
                        .setColor(0x2ecc71)
                        .setTitle(`🎯 Kill Alert: ${killerName} (${killerGuild || 'No Guild'})`)
                        .setDescription(`⚔️ สังหาร **${victimName}** (${victimGuild || 'No Guild'})\n💰 Kill Fame: \`${fame}\``)
                        .setTimestamp(new Date(ev.TimeStamp || Date.now()));
                    await channel.send({ embeds: [embed] }).catch(() => {});
                }
            }
        }

        // 2. ตรวจสอบ Death Trackers
        for (const tracker of targetDeathTrackers) {
            const isMatch = tracker.type === 'player' 
                ? victimName.toLowerCase() === tracker.name.toLowerCase()
                : victimGuild.toLowerCase() === tracker.name.toLowerCase();
            ...
            if (isMatch) {
                ...
                if (channel) {
                    ...
                    const embed = new EmbedBuilder()
                        ...
                        .setDescription(`⚰️ ถูกสังหารโดย **${killerName}** (${killerGuild || 'No Guild'})\n💰 Lost Fame: \`${fame}\``)
                        .setTimestamp(new Date(ev.TimeStamp || Date.now()));
                    await channel.send({ embeds: [embed] }).catch(() => {});
                }
            }
...
async function checkAutoBattles() {
    ...
    // เพิ่มกิลด์จาก /kill และ /death ประเภท guild เข้ามาเช็ก Auto-Battle ด้วยอัตโนมัติ
    ...
    // วิ่งเช็กทั้ง Kill/Death Events และ Auto Battles พร้อมกัน
...
const commands = [
    ...
    new SlashCommandBuilder().setName('kill').setDescription('ตั้งค่าติดตามการฆ่าของผู้เล่นหรือกิลด์ พร้อมเลือกห้องแจ้งเตือน')
    ...
    new SlashCommandBuilder().setName('death').setDescription('ตั้งค่าติดตามการตายของผู้เล่นหรือกิลด์ พร้อมเลือกห้องแจ้งเตือน')
        .addStringOption(o => o.setName('type').setDescription('ประเภทการติดตาม').setRequired(true).addChoices({ name: 'Player', value: 'player' }, { name: 'Guild', value: 'guild' }))
        .addStringOption(o => o.setName('name').setDescription('ชื่อผู้เล่นหรือชื่อ Guild ที่ต้องการติดตาม').setRequired(true))
        .addChannelOption(o => o.setName('channel').setDescription('ห้อง Discord ที่ต้องการให้ส่งรายงานแจ้งเตือน').addChannelTypes(ChannelType.GuildText).setRequired(true)),
...
client.once('clientReady', async () => {
    ...
    setInterval(checkAutoBattles, 60 * 1000); // เช็กข้อมูลทุกๆ 1 นาที
});
