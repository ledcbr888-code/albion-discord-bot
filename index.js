...
async function generateKillDeathCardImage(eventData, isDeath = false) {
    const width = 1000;
    const height = 450;
    const canvas = createCanvas(width, height);
    const ctx = canvas.getContext('2d');

    ctx.fillStyle = isDeath ? '#2c1517' : '#15251c';
    ctx.fillRect(0, 0, width, height);

    ctx.fillStyle = isDeath ? '#e74c3c' : '#2ecc71';
    ctx.font = 'bold 28px sans-serif';
    ctx.fillText(isDeath ? '💀 DEATH REPORT' : '🎯 KILL REPORT', 40, 50);

    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 22px sans-serif';
    ctx.fillText(`Killer: ${eventData.killerName || 'Unknown'} (${eventData.killerGuild || 'No Guild'})`, 40, 110);
    ctx.fillText(`Victim: ${eventData.victimName || 'Unknown'} (${eventData.victimGuild || 'No Guild'})`, 40, 160);

    ctx.fillStyle = '#f1c40f';
    ctx.font = 'bold 24px sans-serif';
    ctx.fillText(`Total Fame: ${Number(eventData.totalFame || 0).toLocaleString()}`, 40, 240);

    ctx.fillStyle = '#95a5a6';
    ctx.font = '18px sans-serif';
    ctx.fillText(`Time: ${formatUTCTime(eventData.eventTime)}`, 40, 290);

    return new AttachmentBuilder(canvas.toBuffer('image/png'), { name: isDeath ? 'death-report.png' : 'kill-report.png' });
}
    ...
    for (const ev of events) {
        ...
        for (const tracker of targetKillTrackers) {
            ...
            if (isMatch) {
                ...
                if (channel) {
                    const eventPayload = { killerName, killerGuild, victimName, victimGuild, totalFame, eventTime };
