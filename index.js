...
client.once('ready', () => {
    clearTimeout(gatewayTimeout);```js
console.log('🔄 Attempting to login to Discord...');
console.log(`🔑 BOT_TOKEN loaded: ${BOT_TOKEN ? 'YES' : 'NO'}`);
console.log(`🔑 BOT_TOKEN length: ${BOT_TOKEN.length}`);
console.log('🌐 Starting Discord Gateway connection...');
...
// ==========================================
// 🚀 DISCORD LOGIN
// ไม่ยิง REST /users/@me ก่อน Login
// เพราะอาจทำให้ Discord/Cloudflare rate-limit (429)
// ==========================================
...
async function loginToDiscord() {
    ...
    while (attempt < maxAttempts) {
        ...
        try {
            console.log(`🔁 Discord Gateway login attempt ${attempt}/${maxAttempts}...`);
            ...
            if (attempt >= maxAttempts) {
                console.error('❌ Maximum Discord login attempts reached.');
                console.error('🔎 Please check BOT_TOKEN and Discord Developer Portal.');
                process.exit(1);
            }

            // Exponential backoff:
            // 10s → 20s → 40s → 60s → 60s...
            const waitSeconds = Math.min(60, 10 * Math.pow(2, attempt - 1));

            console.log(`⏳ Retrying Discord login in ${waitSeconds} seconds...`);
...
client.once('ready', async () => {
    ...
    // Register Slash Commands หลังจาก Gateway พร้อมแล้ว
    try {
        const rest = new REST({ version: '10' }).setToken(BOT_TOKEN);
...
// ==========================================
// ⚠️ DISCORD ERROR / WARNING LOGGING
// ==========================================
...
client.on('debug', info => {
    // แสดงเฉพาะข้อความสำคัญ ป้องกัน Render Log เต็มเร็วเกินไป
    if (
...
// ⏱️ GATEWAY WATCHDOG
// ==========================================
// ไม่ Kill process เมื่อครบ 45 วินาที
// เพราะ Discord.js สามารถ reconnect Gateway ได้เอง
//
// ถ้า Gateway ยังไม่ READY หลังจาก 2 นาที
// แค่แสดงคำเตือนใน Log และปล่อยให้ Discord.js จัดการต่อ

const gatewayWatchdog = setTimeout(() => {
    if (!client.isReady()) {
        console.warn('');
        console.warn('⚠️ Discord Gateway has not reached READY yet.');
        console.warn('⚠️ Waiting for Discord.js automatic connection/reconnect...');
        console.warn('⚠️ BOT PROCESS WILL NOT BE TERMINATED.');
    }
}, 120000);

// ถ้า READY แล้ว ยกเลิก watchdog
client.once('ready', () => {
...
loginToDiscord().catch(error => {
    console.error('❌ Fatal Discord login error:', error);
});
```
