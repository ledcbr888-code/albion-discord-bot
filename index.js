...
async function generateTopPerformanceImage(players) {
    ...
    for (let i = 0; i < players.length; i++) {
        ...
        // แยกการแสดงผลชื่อผู้เล่น และ กิลด์ ออกจากกัน
        let name = p.name;
        if (name.length > 25) name = `${name.slice(0, 22)}...`;
        ctx.fillStyle = '#000';
        ctx.font = 'bold 20px sans-serif';
        ctx.fillText(name, cardX + 94, y + 32);

        if (p.guild) {
            ctx.fillStyle = '#222';
            ctx.font = '14px sans-serif';
            let guildText = `Guild: ${p.guild}`;
            if (guildText.length > 28) guildText = `${guildText.slice(0, 25)}...`;
            ctx.fillText(guildText, cardX + 94, y + 50);
        }

        ctx.fillStyle = '#000';
        ctx.font = 'bold 15px sans-serif';
        ctx.fillText(`${p.type === 'heal' ? 'HEAL' : 'DMG'}  ${Number(p.value || 0).toLocaleString()}  (${percent}%)`, cardX + 94, y + 70);
    }
...
async function processBattleReport(input, targetContext, isMessage = false) {
    try {
        ...
        // 🟢 แยก Guild ออกเป็นคอลัมน์ต่างหาก
        const widths = { name: 15, guild: 13, kills: 8, deaths: 8, fame: 9 };
        const totalWidth = widths.name + widths.guild + widths.kills + widths.deaths + widths.fame;
        ...
        // ส่วนหัวรายงาน (แยกคอลัมน์ Name และ Guild)
        ...
        header += `\x1b[1;37m${'Name'.padEnd(widths.name)}${'Guild'.padEnd(widths.guild)}${centerString('Kills', widths.kills)}${centerString('Deaths', widths.deaths)}${centerString('Fame', widths.fame)}\x1b[0m\n`;
        header += `\x1b[30m${subDivider}\x1b[0m\n`;

        // ส่วนท้ายรายงาน
        let footer = `\x1b[30m${subDivider}\x1b[0m\n`;
        footer += `\x1b[1;37m${'TOTAL'.padEnd(widths.name + widths.guild)}\x1b[32m${centerString(totalKills, widths.kills)}\x1b[31m${centerString(totalDeaths, widths.deaths)}\x1b[33m${centerString(formatFame(totalFame), widths.fame)}\x1b[0m\n`;
        ...
        if (rows.length === 0) {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                const guildName = (p.guild || '-').slice(0, widths.guild - 1).padEnd(widths.guild);
                ...
                const line = `${name}\x1b[36m${guildName}\x1b[0m${p.kills > 0 ? `\x1b[32m${kills}\x1b[0m` : `\x1b[30m${kills}\x1b[0m`}${p.deaths > 0 ? `\x1b[31m${deaths}\x1b[0m` : `\x1b[30m${deaths}\x1b[0m`}${p.fame > 0 ? `\x1b[33m${fame}\x1b[0m` : `\x1b[30m${fame}\x1b[0m`}\n`;
...
// 🟢 ตัดคำสั่ง /regear, /shutdown, /stat ออกแล้ว
const commands = [
