...
// โหลดรูปไอคอน Fame Buff จาก Albion Online Render API
async function loadFameIcon() {
    try {
        ...
        return await loadImage(fameIconUrl);
    } catch (err) {
        console.error('Failed to load Fame icon:', err);
        return null;
    }
}
...
async function generatePlayerWeaponReportImage(players, battleInfo = {}) {
    ...
    // โหลดรูปไอคอน Fame Buff เตรียมไว้
    const fameImg = await loadFameIcon();

    // 1. เรียงลำดับตาม Fame/Kills
    const sortedPlayers = [...players].sort((a, b) => 
        (b.fame || 0) - (a.fame || 0) || (b.kills || 0) - (a.kills || 0)
    );
    ...
    // 3. กำหนดขนาด Layout ใหม่ (แถวละ 7 คน + การ์ดใหญ่ขึ้น)
    const playersPerRow = 7;
    const cardWidth = 165;       // ขยายความกว้างการ์ด
    const cardHeight = 150;      // ขยายความสูงการ์ด
    const gapX = 14;
    const gapY = 16;
    const padding = 28;
    const headerTopHeight = 70; 
    const statsBarHeight = 85;  
    const gridOffsetY = padding + headerTopHeight + statsBarHeight + 25;

    const columns = Math.min(playersPerRow, sortedPlayers.length);
    const rows = Math.ceil(sortedPlayers.length / playersPerRow);

    const width = Math.max(1280, padding * 2 + columns * cardWidth + (columns - 1) * gapX);
    ...
    // === BACKGROUND (สไตล์ Cyber Dark / Grid) ===
    ctx.fillStyle = '#06080d';
    ctx.fillRect(0, 0, width, height);

    // Radial Gradient เรืองแสง
    const bgGlow = ctx.createRadialGradient(width / 2, height / 2, 100, width / 2, height / 2, width * 0.8);
    bgGlow.addColorStop(0, '#101726');
    bgGlow.addColorStop(0.6, '#080c14');
    bgGlow.addColorStop(1, '#040508');
    ctx.fillStyle = bgGlow;
    ctx.fillRect(0, 0, width, height);

    // วาดเส้นตาราง Grid Pattern
    ...
    ctx.fillText('BATTLE REPORT', padding + 10, padding + 36);
    ctx.shadowBlur = 0;

    ctx.fillStyle = '#64748b';
    ctx.font = 'bold 12px sans-serif';
    const serverTimeText = `EAST SERVER   |   ${formatUTCTime(battleInfo.battleTime || Date.now())}   |   MATCH ID: ${battleInfo.matchId || 'N/A'}`;
    ctx.fillText(serverTimeText, padding + 10, padding + 56);

    // Players Badge ด้านขวาบน
    const badgeX = width - padding - 140;
    const badgeY = padding + 5;
    ctx.fillStyle = '#0f172a';
    ctx.beginPath();
    ctx.roundRect(badgeX, badgeY, 140, 52, 10);
    ctx.fill();
    ...
    ctx.fillText(`👥 ${sortedPlayers.length}`, badgeX + 32, badgeY + 28);
    ctx.fillStyle = '#94a3b8';
    ctx.font = 'bold 10px sans-serif';
    ctx.fillText('PLAYERS', badgeX + 54, badgeY + 42);

    // === 2. STATS BAR (5 CARDS) ===
    const statBoxWidth = (width - padding * 2 - 4 * 14) / 5;
    const statBoxY = padding + headerTopHeight;
    const statConfigs = [
        { label: 'TOTAL KILLS', val: totalKills.toLocaleString(), icon: '⚔️', color: '#c084fc' },
        { label: 'TOTAL DEATHS', val: totalDeaths.toLocaleString(), icon: '💀', color: '#f87171' },
        { label: 'TOTAL FAME', val: formatFame(totalFame), icon: '👑', color: '#fbbf24' },
        { label: 'TOP KILLER', val: topKiller.name.slice(0, 11), sub: `${topKiller.kills} KILLS`, icon: '👾', color: '#e879f9' },
        { label: 'MVP (TOP FAME)', val: mvp.name.slice(0, 11), sub: `${formatFame(mvp.fame)} FAME`, icon: '🏆', color: '#facc15' }
    ];

    statConfigs.forEach((st, idx) => {
        const sx = padding + idx * (statBoxWidth + 14);
        
        ctx.fillStyle = '#0f172a';
        ctx.beginPath();
        ctx.roundRect(sx, statBoxY, statBoxWidth, statsBarHeight - 10, 10);
        ctx.fill();
        ...
        ctx.fillText(st.icon, sx + 18, statBoxY + 42);

        ctx.fillStyle = '#64748b';
        ctx.font = 'bold 11px sans-serif';
        ctx.fillText(st.label, sx + 58, statBoxY + 24);

        ctx.fillStyle = '#f8fafc';
        ctx.font = 'bold 18px sans-serif';
        ctx.fillText(String(st.val), sx + 58, statBoxY + 46);

        if (st.sub) {
            ctx.fillStyle = st.color;
            ctx.font = 'bold 11px sans-serif';
            ctx.fillText(st.sub, sx + 58, statBoxY + 62);
        }
    });

    // === 3. PLAYER CARDS GRID (7 คนต่อแถว / รูปใหญ่ขึ้น) ===
    ...
    sortedPlayers.forEach((p, i) => {
        ...
        const isMVP = i === 0;
        const isFeeder = p.deaths > 0 && p.kills === 0;

        // Background การ์ด
        ctx.fillStyle = isMVP ? '#131c2e' : '#0d131f';
        ctx.beginPath();
        ...
        // Rank Badge
        ctx.fillStyle = isMVP ? '#f59e0b' : '#334155';
        ctx.beginPath();
        ctx.roundRect(x + 6, y + 6, 26, 16, 4);
        ctx.fill();
        ctx.fillStyle = isMVP ? '#000000' : '#f1f5f9';
        ctx.font = 'bold 10px sans-serif';
        ctx.fillText(rankNum, x + 10, y + 18);

        // Weapon Icon Container (ขนาดรูปอาวุธ 64x64 px ใหญ่ขึ้น ดูง่ายขึ้น)
        const imgSize = 64;
        ...
        ctx.fill();
        ctx.strokeStyle = isMVP ? '#d97706' : '#334155';
        ctx.lineWidth = 1;
        ctx.stroke();

        if (iconImages[i]) {
            try {
                ctx.drawImage(iconImages[i], imgX + 3, imgY + 3, imgSize - 6, imgSize - 6);
            } catch (_) {}
        }
        ...
        ctx.fillText(name, x + (cardWidth - nameWidth) / 2, y + 96);

        // K / D Stats
        const kVal = p.kills || 0;
        const dVal = p.deaths || 0;
        ctx.font = 'bold 13px monospace';

        const kText = `${kVal}`;
        const slashText = ` / `;
        const dText = `${dVal}`;
        const totalStatWidth = ctx.measureText(kText + slashText + dText).width;
        let startStatX = x + (cardWidth - totalStatWidth) / 2;

        ctx.fillStyle = kVal > 0 ? '#c084fc' : '#94a3b8'; 
        ctx.fillText(kText, startStatX, y + 116);
        startStatX += ctx.measureText(kText).width;

        ctx.fillStyle = '#475569'; 
        ctx.fillText(slashText, startStatX, y + 116);
        startStatX += ctx.measureText(slashText).width;

        ctx.fillStyle = dVal > 0 ? '#ef4444' : '#94a3b8'; 
        ctx.fillText(dText, startStatX, y + 116);

        // Fame Badge ด้านล่าง (ใส่รูปไอคอน T6_GVGSEASONREWARD_FAMEBUFF_SPELL จริง)
        const fameText = formatFame(p.fame || 0);
        ctx.font = 'bold 12px sans-serif';
        const textWidth = ctx.measureText(fameText).width;
        
        const fameIconSize = 16;
        const gapBetween = 4;
        const totalFameBadgeWidth = fameIconSize + gapBetween + textWidth;
        const fameStartX = x + (cardWidth - totalFameBadgeWidth) / 2;

        if (fameImg) {
            try {
                ctx.drawImage(fameImg, fameStartX, y + 125, fameIconSize, fameIconSize);
            } catch (_) {}
        }

        ctx.fillStyle = '#fbbf24';
        ctx.fillText(fameText, fameStartX + fameIconSize + gapBetween, y + 138);
    });
    ...
    // เปลี่ยนข้อความมุมขวาล่างเป็น BY BOTBOSS
    ctx.font = 'bold 12px sans-serif';
    ctx.fillStyle = '#a855f7';
    const poweredText = 'BY BOTBOSS';
    const pwWidth = ctx.measureText(poweredText).width;
    ctx.fillText(poweredText, width - padding - pwWidth, footerY);
