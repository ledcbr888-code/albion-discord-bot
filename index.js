...
// ฟังก์ชันสร้างรูปตารางสรุปกิลด์เฉพาะ Top 5 (Guild Summary Canvas)
async function generateGuildSummaryImage(guildsData) {
    if (!guildsData || !guildsData.length) return null;
    
    // ตัดเลือกเฉพาะ Top 5 อันดับแรกที่ Fames สูงสุด
...
async function buildBattleReportPayload(matchId, customTargetGuilds = []) {
    ...
    // Enrich the official battle players with MainHand from AlbionBB when the
    // official battle snapshot does not include Equipment.MainHand.
    try {
        const weaponMap = await fetchAlbionBBWeaponMap(matchId);
        if (weaponMap.size) {
            rawPlayers = rawPlayers.map(p => {
                if (p.weapon) return p;
                const info = weaponMap.get(p.name.trim().toLowerCase());
    ...
    try {
        if (apiData.guilds) {
            ...
            // Auto-Battle report: show only the exact tracked Guild.
            if (guildNamesList.length > 0) {
    ...
    try {
        for (const config of autoBattleConfigs) {
            ...
            try {
                ...
                const result = await buildBattleReportPayload(latestMatchId, [config.targetGuild]);
