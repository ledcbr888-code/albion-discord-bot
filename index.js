...
function parseDailyBonusBlock(block) {
    ...
    // Homepage card format is generally:
    //   <category> <best city> +15% +10%
    // Keep this deliberately permissive because category names can contain spaces,
    // apostrophes, hyphens, etc.
    const pairRe = /([A-Za-z][A-Za-z0-9'’&+().\/-]*(?:\s+[A-Za-z][A-Za-z0-9'’&+().\/-]*){0,7})\s+([A-Z][A-Za-z'’-]*(?:\s+[A-Z][A-Za-z'’-]*){0,3})\s+\+(\d+)%\s+\+(\d+)%/g;
    let match;
...
async function generateDailyBonusEmbed(serverChoice) {
    ...
    return new EmbedBuilder()
        ...
        .addFields(
            ...
            {
                name: '📝 **แหล่งข้อมูล**',
                value: 'AO-SAGE — ข้อมูลโบนัสที่บันทึกหลังการรีเซ็ตประจำวัน',
                inline: false
            },
            {
                name: '⏰ **เวลาการรีเซ็ต**',
                value: '⏳ **10:00 UTC (17:00 น. ไทย)**',
                inline: false
            }
        )
